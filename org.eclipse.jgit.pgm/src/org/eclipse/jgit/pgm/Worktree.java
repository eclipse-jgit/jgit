...
class Worktree extends TextBuiltin {
	...
	@org.kohsuke.args4j.Option(name = "--checkout")
	...
	protected void run() {
		try (Git git = new Git(db)) {
			...
			case "add": //$NON-NLS-1$
				...
				addCmd.setCheckout(checkout && !noCheckout);
				addCmd.setLock(lock);
			...
			case "lock": //$NON-NLS-1$
				if (arg1 == null) {
					throw die("Usage: jgit worktree lock [--reason <string>] <path>"); //$NON-NLS-1$
				}
				git.worktreeLock().setPath(arg1).setReason(lockReason).call();
				break;
