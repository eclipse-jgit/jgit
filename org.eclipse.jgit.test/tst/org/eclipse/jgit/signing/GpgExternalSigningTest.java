...
public class GpgExternalSigningTest extends RepositoryTestCase {
	...
	public void testCommitSigning() throws Exception {
		...
		when(mockRunner.run(any(), any(), (GpgProcessConfiguration) any()))
				.thenReturn(result);
		when(mockRunner.run(any(), any(), anyBoolean(),
				(GpgProcessConfiguration) any())).thenReturn(result);
	...
	public void testVerifierParsing() throws Exception {
		...
		when(mockRunner.run(any(), any(), anyBoolean(), any()))
				.thenReturn(result);
	...
	@Test
	public void testTerminalPromptRejection() throws Exception {
		GpgProcessRunner mockRunner = mock(GpgProcessRunner.class);

		// Stub the runner to throw IOException when rejection is expected
		String errorMsg = "GPG attempted to use a terminal prompt: [GNUPG:] PINENTRY_LAUNCHED 12345 tty 1.2.0";
		when(mockRunner.run(any(), any(), any()))
				.thenThrow(new IOException(errorMsg));

		TestGpgBinarySigner signer = new TestGpgBinarySigner(mockRunner);
		signer.setProcessConfiguration(new GpgProcessConfiguration()
				.setRejectTerminalPrompt(true));
		Signers.set(GpgFormat.OPENPGP, signer);
		...
		try (Git git = new Git(db)) {
			...
			try {
				git.commit().setMessage("Signed").setSign(true).call();
				fail("Expected JGitInternalException due to terminal prompt");
			} catch (JGitInternalException e) {
				assertTrue(e.getMessage().contains("terminal prompt"));
				assertTrue(e.getMessage().contains("tty"));
			}
		}
	}

	@Test
	public void testTerminalPromptAllowedByDefault() throws Exception {
		byte[] fakeSig = "FAKE_SIG".getBytes(StandardCharsets.UTF_8);
		String statusOutput = "[GNUPG:] PINENTRY_LAUNCHED 12345 tty 1.2.0\n";

		TemporaryBuffer.Heap stdout = new TemporaryBuffer.Heap(Integer.MAX_VALUE);
		stdout.write(fakeSig);
		TemporaryBuffer.Heap stderr = new TemporaryBuffer.Heap(Integer.MAX_VALUE);
		stderr.write(statusOutput.getBytes(StandardCharsets.UTF_8));

		GpgProcessRunner mockRunner = mock(GpgProcessRunner.class);
		FS.ExecutionResult result = new FS.ExecutionResult(stdout, stderr, 0);

		// Default configuration (null or reject=false) should not throw
		when(mockRunner.run(any(), any(), any())).thenReturn(result);
	...
	@Test
	public void testGuiPinentryAllowed() throws Exception {
		byte[] fakeSig = "FAKE_SIG".getBytes(StandardCharsets.UTF_8);
		String statusOutput = "[GNUPG:] PINENTRY_LAUNCHED 12345 gtk 1.2.0\n";
		...
		// Even with rejection enabled, gtk should pass
		when(mockRunner.run(any(), any(), any())).thenReturn(result);
