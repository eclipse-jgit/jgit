...
public class CherryPickCommandTest extends RepositoryTestCase {
	...
	public void testSequentialCherryPick()
		...
		try (Git git = new Git(db)) {
			...
			assertFalse(new File(db.getDirectory(),
					Constants.SEQUENCER_TODO_FILE).exists());
	...
	public void testSequencerStateWrittenOnMultiCommitConflict()
		...
		try (Git git = new Git(db)) {
			writeTrashFile("a", "first line\nsec. line\nthird line\n");
			git.add().addFilepattern("a").call();
			RevCommit firstCommit = git.commit().setMessage("create a")
					.call();
			...
			git.add().addFilepattern("b").call();
			RevCommit unrelatedB = git.commit().setMessage("create b")
					.call();
			...
			CherryPickResult result = git.cherryPick()
					.include(conflictingA.getId())
					.include(unrelatedB.getId()).call();
			...
			assertEquals("pick " + conflictingA.abbreviate(7).name()
					+ " conflicting a\n"
					+ "pick " + unrelatedB.abbreviate(7).name()
					+ " create b\n", todo);
