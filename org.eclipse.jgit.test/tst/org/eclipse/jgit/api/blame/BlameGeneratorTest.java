...
public class BlameGeneratorTest extends RepositoryTestCase {
	...
	public void cache_tipInCache() throws Exception {
		...
		c4Regions.add(new CacheRegion(FILE, c1, author, 0, 1));
		c4Regions.add(new CacheRegion(FILE, c4, author, 1, 2));
		c4Regions.add(new CacheRegion(FILE, c3, author, 2, 3));
		c4Regions.add(new CacheRegion(FILE, c4, author, 3, 4));
	...
	public void cache_intermediateInCache() throws Exception {
		...
		c2Regions.add(new CacheRegion(FILE, c1, author, 0, 1));
		c2Regions.add(new CacheRegion(FILE, c2, author, 1, 3));
		c2Regions.add(new CacheRegion(FILE, c1, author, 3, 4));
	...
	public void cache_compareWithAndWithout() throws Exception {
		...
		try (BlameGenerator genWithCache = new BlameGenerator(db, FILE, cache)) {
		...
		try (BlameGenerator generator = new BlameGenerator(db, FILE)) {
			...
			while (generator.next()) {
				regions.add(new CacheRegion(FILE,
						...
						generator.getSourceAuthor(), generator.getResultStart(),
	...
	private List<EmittedRegion> consume(BlameGenerator generator) throws IOException {
	...
	private static void assertListsEquals(List<EmittedRegion> a, List<EmittedRegion> b) {
		assertEquals(a.size(), b.size());
		for (int i = 0; i < a.size(); i++) {
			assertEquals(String.format("List differ in element %d", i), a.get(i), b.get(i));
		}
	}
}
