...
public class CommitAndLogCommandTest extends RepositoryTestCase {
	...
	public void testSomeCommits() throws Exception {
		...
		try (Git git = new Git(repository)) {
			...
			PersonIdent defaultCommitter = new PersonIdent(repository);
	...
	public void testMergeEmptyBranches() throws Exception {
		try (Git git = new Git(repository)) {
			git.commit().setMessage("initial commit").call();
			RefUpdate r = repository.updateRef("refs/heads/side");
			r.setNewObjectId(repository.resolve(Constants.HEAD));
			assertEquals(r.forceUpdate(), RefUpdate.Result.NEW);
			RevCommit second = git.commit().setMessage("second commit")
					.setCommitter(committer).call();
			repository.updateRef(Constants.HEAD).link("refs/heads/side");
			RevCommit firstSide = git.commit().setMessage("first side commit")
					.setAuthor(author).call();

			write(new File(repository.getDirectory(), Constants.MERGE_HEAD),
					ObjectId.toString(repository.resolve("refs/heads/master")));
			write(new File(repository.getDirectory(), Constants.MERGE_MSG), "merging");
	...
	public void testAddUnstagedChanges() throws Exception {
		...
		try (Git git = new Git(repository)) {
			...
			TreeWalk tw = TreeWalk.forPath(repository, "a.txt", commit.getTree());
			...
			tw = TreeWalk.forPath(repository, "a.txt", commit.getTree());
			assertEquals("6b584e8ece562ebffc15d38808cd6b98fc3d97ea",
					tw.getObjectId(0).getName());
