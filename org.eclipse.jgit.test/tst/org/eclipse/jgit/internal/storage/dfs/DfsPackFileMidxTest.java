...
public class DfsPackFileMidxTest {
	...
	public void midx_findOffset() throws IOException {
		...
		try (DfsReader ctx = db.getObjectDatabase().newReader()) {
			...
			PackOffset po = midx.getOffsetCalculator().decode(posOne);
			assertEquals(12, po.getOffset());
			int packOne = po.getPackId();
			assertTrue(packOne >= 0 && packOne < 3);

			long posTwo = midx.findOffset(ctx, o2);
			po = midx.getOffsetCalculator().decode(posTwo);
			assertEquals(12, po.getOffset());
			int packTwo = po.getPackId();
			assertTrue(packTwo >= 0 && packTwo < 3);
			assertTrue(packTwo != packOne);

			long posThree = midx.findOffset(ctx, o3);
			po = midx.getOffsetCalculator().decode(posThree);
			assertEquals(12, po.getOffset());
			int packThree = po.getPackId();
			assertTrue(packThree >= 0 && packThree < 3);
			assertTrue(packThree != packOne);
			assertTrue(packThree != packTwo);
	...
	@Test
	public void voffsetcalculator_firstObjectPerPack() {
		...
		DfsPackFileMidx.VOffsetCalculator calc = DfsPackFileMidx.VOffsetCalculator
