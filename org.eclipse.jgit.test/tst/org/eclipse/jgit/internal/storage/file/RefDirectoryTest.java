...
public class RefDirectoryTest extends LocalDiskRepositoryTestCase {
	...
	@Test
	public void testUnpeeledPackFileWithUnPeeledTagsIsPreservedAsUnpeeled() throws Exception {
		writePackedRefs("# pack-refs with: sorted \n" +
				v1_0.name() + " refs/tags/peeled\n" +
				"^" + v1_0.getObject().name() + "\n" +
				B.name() + " refs/tags/unpeeled"
		);
		...
		assertFalse(refdir.getPackedRefs().traits().contains(Trait.PEELED));
		assertTrue(refdir.getPackedRefs().get("refs/tags/peeled").isPeeled());
		assertFalse(refdir.getPackedRefs().get("refs/tags/unpeeled").isPeeled());
	}
	...
	@Test
	public void testPackingRefsPreservesFullyPeeledRefs() throws Exception {
		String peeledPackedRef = "refs/tags/peeled";
		RevTag peeledPackedTag = repo.tag("peeled", A);
		writePackedRefs(
			"# pack-refs with: sorted fully-peeled\n" +
					// peeled tag
					peeledPackedTag.name() + " %s\n".formatted(peeledPackedRef) +
					"^" + peeledPackedTag.getObject().name() + "\n"
		);

		RevTag updatedTag = repo.tag("update", B);
		writeLooseRef(peeledPackedRef, updatedTag);
		...
		Ref tag = refdir.getPackedRefs().get(peeledPackedRef);
		assertNotNull(tag);
		assertTrue(tag.isPeeled());
		assertEquals(updatedTag.getObject().getId(), tag.getPeeledObjectId());
	}

	@Test
	public void testPackingRefsDoesFullyPeelNewRefs() throws Exception {
		String peeledPackedRef = "refs/tags/peeled";
		String looseRef = "refs/tags/loose";
		RevTag peeledPackedTag = repo.tag("peeled", B);
		RevTag looseTag = repo.tag("loose", B);
		...
		writeLooseRef(looseRef, looseTag); // loose tag
		...
		Ref tag = refdir.getPackedRefs().get(looseRef);
		assertNotNull(tag);
		assertTrue(tag.isPeeled());
		assertEquals(looseTag.getObject(), tag.getPeeledObjectId());
	}


	@Test
	public void testPackRefsDoesNotPeelUnpeeledPackedTag() throws Exception {
		writePackedRefs(
		...
		File packedRefsFile = new File(diskRepo.getCommonDirectory(), "packed-refs");
		String firstLine = read(packedRefsFile).split("\n")[0];
		assertFalse("packed-refs should NOT have header with fully-peeled",
				firstLine.contains(" fully-peeled"));

		Ref tag = refdir.getPackedRefs().get("refs/tags/v1.0");
		assertNotNull("tag ref should be packed", tag);
		assertFalse("tag should NOT be peeled", tag.isPeeled());
	}
