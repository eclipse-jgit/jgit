...
public class GcSinceBitmapStatisticsTest extends GcTestCase {
	...
	public void testShouldNotReportNewTagsOrCommitsObjectsWhenRepositoryProgressesWithNoHeads()
		...
		gc.gc().get();
		assertEquals(5L, gc.getStatistics().numberOfObjectsSinceBitmap);
		assertEquals(0L,
				gc.getStatistics().numberOfTagsHeadsObjectsSinceBitmap);

		// progress & pack
		addCommit(parent, refsMeta);
		assertEquals(6L, gc.getStatistics().numberOfObjectsSinceBitmap);
		assertEquals(0L,
				gc.getStatistics().numberOfTagsHeadsObjectsSinceBitmap);

		tr.packAndPrune();
		assertEquals(9L, gc.getStatistics().numberOfObjectsSinceBitmap);
		assertEquals(0L,
				gc.getStatistics().numberOfTagsHeadsObjectsSinceBitmap);
	}
