...
public class BasePackWriterTest extends SampleDataRepositoryTestCase {
	...
	@Test
	public void testUnshallowFetchShallowAncestorDepth2() throws Exception {
		//     [unshallow-branch]
		//         |
		//         v
	...
	private static PackIndex writePack(FileRepository repo,
		...
		try (RevWalk walk = new RevWalk(repo)) {
			return writePack(repo, walk, 0, want, NONE, excludeObjects, false, false);
		}
	}
	...
	private static PackIndex writeShallowPack(FileRepository repo, int depth,
		...
		try (DepthWalk.RevWalk walk = new DepthWalk.RevWalk(repo, depth - 1)) {
			...
			return writePack(repo, walk, depth, want, have, EMPTY_ID_SET, false, false);
		}
	}
	...
	private static PackIndex writeUnshallowPack(FileRepository repo, int depth,
		...
		try (DepthWalk.RevWalk walk = new DepthWalk.RevWalk(repo, depth)) {
			return writePack(repo, walk, depth, want, have, EMPTY_ID_SET, true, false);
		}
	}

	private static PackIndex writePack(FileRepository repo, RevWalk walk,
			int depth, Set<? extends ObjectId> want,
			Set<? extends ObjectId> have, Set<ObjectIdSet> excludeObjects, boolean unshallow, boolean thin)
					throws IOException {
		try (PackWriter pw = new PackWriter(repo)) {
