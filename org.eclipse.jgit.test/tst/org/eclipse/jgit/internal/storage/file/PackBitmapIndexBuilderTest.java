...
public class PackBitmapIndexBuilderTest {
	...
	private static int COMMIT_COUNT = 20
	...
	public void testPackBitmapIndexBuilder_inflateXorCompressedBitmaps() {
		...
		List<StoredEntry> builderEntries = builder.getCompressedBitmaps();
		...
		for (int i = 0; i < builderEntries.size(); i++) {
			...
			if (i >= PackBitmapIndexBuilder.MAX_XOR_OFFSET_SEARCH) {
				xorOffset = random
						.nextInt(PackBitmapIndexBuilder.MAX_XOR_OFFSET_SEARCH
								- 1)
						+ 1;
		...
		List<StoredEntry> actualBitmaps = PackBitmapIndexBuilder
				.getEwahCompressedBitmaps(xorCompressedEntries);
		assertEquals("correct number of bitmaps are returned",
				COMMIT_BITMAPS.size(), actualBitmaps.size());

		for (StoredEntry actualBitmap : actualBitmaps) {
			ObjectId objectId = builder.getObject((int) actualBitmap.getObjectId());
	...
	private static class RandomBitmap {
		private static final int BITMAP_WORDS = 2000;
