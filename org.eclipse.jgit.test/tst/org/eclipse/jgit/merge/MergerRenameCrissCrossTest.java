...
public class MergerRenameCrissCrossTest extends RepositoryTestCase {
   ...
   * <p>
   * <p>
   * ORIGINAL_FILENAME is renamed to different paths in A1 and A2 ("branch-to-merge").
   * <p>
   * A2 is merged into "master" and "second-branch". The rename/rename merge conflict is resolved
   * manually, results in C4 and C3.
   * <p>
   * While merging C3 and C4, A1 and A2 are the base commits found by the recursive merge that
   * have the rename/rename conflict.
   */
  public MergeResult setUpRenameRenameConflictInVirtualAncestor(String resolveName1, String resolveContent1, String resolveName2, String resolveContent2)
      throws Exception {
    ...
    MergeResult mergeResult = git.merge().include(commitA2).setStrategy(MergeStrategy.RECURSIVE).call();
    assertEquals(mergeResult.getNewHead(), null);
    assertEquals(mergeResult.getMergeStatus(), MergeStatus.CONFLICTING);

    // Resolve the conflict manually, merge with the A1's rename name and current branch content
    git.rm().addFilepattern(ORIGINAL_FILENAME).call();
    ...
    mergeResult = git.merge().include(commitA2).setStrategy(MergeStrategy.RECURSIVE).call();
    assertEquals(mergeResult.getNewHead(), null);
    assertEquals(mergeResult.getMergeStatus(), MergeStatus.CONFLICTING);
    ...
    git.rm().addFilepattern(RENAME_FILENAME1).call();
    git.rm().addFilepattern(RENAME_FILENAME2).call();
    // Resolve the conflict manually, merge with current branch's name content
  ...
  // Since rename was not dedicated, both files are present in the final version
  @Theory
  public void checkRenameMergeConflictInVirtualAncestor_resolveToDifferentNamesInChildren_renameNotDetected(boolean keepInOurs)
      throws Exception {
    MergeResult mergeResult = setUpRenameRenameConflictInVirtualAncestor(
        keepInOurs ? RENAME_FILENAME1 : RENAME_FILENAME2, SLIGHTLY_MODIFIED_CONTENT1,
        keepInOurs ? RENAME_FILENAME2 : RENAME_FILENAME1, SLIGHTLY_MODIFIED_CONTENT2);
    assertFalse(check(ORIGINAL_FILENAME));
    assertEquals(SLIGHTLY_MODIFIED_CONTENT1, read(keepInOurs ? RENAME_FILENAME1 : RENAME_FILENAME2));
    assertEquals(SLIGHTLY_MODIFIED_CONTENT2, read(keepInOurs ? RENAME_FILENAME2 : RENAME_FILENAME1));
  ...
  public void checkRenameMergeConflictInVirtualAncestor_resolveToSameName_conflicting()
      throws Exception {
    MergeResult mergeResult = setUpRenameRenameConflictInVirtualAncestor(RENAME_FILENAME2,
        SLIGHTLY_MODIFIED_CONTENT2, RENAME_FILENAME2, SLIGHTLY_MODIFIED_CONTENT1);
    assertEquals(mergeResult.getMergeStatus(), MergeStatus.CONFLICTING);
    assertFalse(check(ORIGINAL_FILENAME));
    assertFalse(check(RENAME_FILENAME1));
    // Since virtual base did not have the source file for content merge, the three-way content merge resulted in conflict.
    assertTrue(read(RENAME_FILENAME2).contains(String.format("<<<<<<< HEAD\n%s\n"
