...
public class MergerTest extends RepositoryTestCase {
	 ...
	 *
	 * Those conflicts should be ignored, otherwise the found base can not be used
	 * by the RecursiveMerger.
	 *
	 */
	@Theory
	public void checkFileDirMergeConflictInVirtualAncestor_NoConflictInChildren(MergeStrategy strategy)
			throws Exception {
		if (!strategy.equals(MergeStrategy.RECURSIVE)) {
			return;
		}
		...
		git.add().addFilepattern("a").call();
		RevCommit commitC1M = git.commit().setMessage("Child 1 on master").call();
		...
		git.add().addFilepattern("a").call();
		RevCommit commitC2S = git.commit().setMessage("Child 2 on second-branch").call();
		...
		writeTrashFile("a", "merge conflict resolution");
		git.add().addFilepattern("a").call();
		RevCommit commitC4M = git.commit().setMessage("Child 4 on master - resolve merge conflict").call();
	...
	@Theory
	public void checkFileDirMergeConflictInVirtualAncestor_ConflictInChildren(MergeStrategy strategy) throws Exception {
		if (!strategy.equals(MergeStrategy.RECURSIVE)) {
			return;
		}
