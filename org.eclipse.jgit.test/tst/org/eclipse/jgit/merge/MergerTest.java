...
public class MergerTest extends RepositoryTestCase {
	...
	@Theory
	public void checkContentMergeConflict_fileBecomesDir(MergeStrategy strategy)
			throws Exception {
		Git git = Git.wrap(db);
		...
		try (ObjectInserter ins = db.newObjectInserter()) {
			...
			assertTrue(success);
			assertTrue(merger.getModifiedFiles().contains("file"));
			assertTrue(merger.getModifiedFiles().contains("file/file"));
		}
	}
