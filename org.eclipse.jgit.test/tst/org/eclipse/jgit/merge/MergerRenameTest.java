 ...
 * </pre>
 * <p>means base commit B contains test/file1 with content X_1.
 * <p> X_2 is the content, modified form X_1, Y is entirely unrelated to X_1(2) content.
 *
 * <p>All the test DataPoints below contain the mappings source (original) File -> target (rename) File
 */
...
public class MergerRenameTest extends RepositoryTestCase {
  ...
  public void enableRename() throws IOException, ConfigInvalidException {
    ...
    config.setString(ConfigConstants.CONFIG_MERGE_SECTION, null, ConfigConstants.CONFIG_KEY_RENAMES, "true");
    config.save();
  }
  ...
  private RevCommit setUpMerge(Git git, Map<String, String> originalFilesToContents,  Map<String, String> oursFilesToContents,  Map<String, String> theirsFilesToContents) throws Exception {
    // master
  ...
  private MergeResult mergeRename(MergeStrategy strategy, Map<String, String> originalFilesToContents,  Map<String, String> oursFilesToContents,  Map<String, String> theirsFilesToContents) throws Exception {
    Git git = Git.wrap(db);
    RevCommit mergeCommit  = setUpMerge(git, originalFilesToContents, oursFilesToContents, theirsFilesToContents);
  ...
  /**
   *  Testcase 3.1
   *  <p> This test checks that the files, are involved as a source of rename are not accidentally dropped,and it does not result in collision.
   *  <pre>
   *    B: test/file1 X_1
   *    O: test/file1 X_2
   *    T: test/file2 X_1, test/file1 Y
  ...
  /**
   * SECTION 4
   * <p>Tests the behaviour for dirty index and working tree
   */

  @Theory
  public void checkRenameRename_dirtyIndex_fail(MergeStrategy strategy, boolean dirtyOriginal, boolean dirtyOurs, boolean dirtyTheirs)
      throws Exception {
    ...
    RevCommit mergeCommit = setUpMerge(git, Map.of(originalFilename, content), Map.of(renameFilename1, content), Map.of(renameFilename2, content));
    ...
    // Merge master into second-branch
    MergeResult mergeResult = git.merge().include(mergeCommit).setStrategy(strategy).call();
    assertEquals(mergeResult.getMergeStatus(), MergeStatus.FAILED);

    assertEquals(mergeResult.getFailingPaths(), Map.of(failingPath, DIRTY_INDEX));
  ...
  @Theory
  public void checkRenameRename_dirtyWorkingTree_fail(MergeStrategy strategy, boolean dirtyOriginal, boolean dirtyOurs, boolean dirtyTheirs)
      throws Exception {
    Git git = Git.wrap(db);
    String content = "Content";
    String originalFilename = "file1";
    ...
    if(dirtyOurs || dirtyTheirs) {
      assertEquals(mergeResult.getMergeStatus(), MergeStatus.FAILED);
      assertEquals(mergeResult.getFailingPaths(), Map.of(failingPath, DIRTY_WORKTREE));
    } else {
      // if unrelated file was modified in working tree, it won't be overwritten by merge,
  ...
  @Theory
  public void checkRenameModify_dirtyIndex_fail(MergeStrategy strategy, boolean isRenameInOurs, boolean dirtyOriginal, boolean dirtyRename)
      throws Exception {
    ...
    Map<String, String> filesWithRename = Map.of(renameFilename, originalContent);
    Map<String, String> filesWithModify = Map.of(originalFilename, slightlyModifiedContent);
    RevCommit mergeCommit = setUpMerge(git, Map.of(originalFilename, originalContent), isRenameInOurs? filesWithRename: filesWithModify, isRenameInOurs?filesWithModify: filesWithRename);
  ...
  @Theory
  public void checkRenameModify_dirtyWorkingTree_renameInTheirs_fail(MergeStrategy strategy, boolean isRenameInOurs, boolean dirtyOriginal, boolean dirtyRename)
      throws Exception {
    if(!dirtyOriginal && !dirtyRename){
      return;
    }
  ...
  @Theory
  public void checkRenameModify_dirtyWorkingTree_renameInOurs_merged(MergeStrategy strategy)
      throws Exception {
    Git git = Git.wrap(db);
    String originalContent = "a\nb\nc";
    String slightlyModifiedContent = "a\nb\nb";
    ...
    Map<String, String> filesWithRename = Map.of(renameFilename, originalContent);
    Map<String, String> filesWithModify = Map.of(originalFilename, slightlyModifiedContent);
    RevCommit mergeCommit = setUpMerge(git, Map.of(originalFilename, originalContent),
        filesWithRename, filesWithModify);
    ...
    MergeResult mergeResult = git.merge().include(mergeCommit).setStrategy(strategy).call();
    assertEquals(mergeResult.getMergeStatus(), MergeStatus.MERGED);
    // This file is just the regular local untracked file, not involved into merge
    // The new content is written to renameFilename and there is no need to delete originalFilename
  ...
  /**
   *  Same as Testcase 3.1, with dirty work tree preventing the file from being added.
   */
  @Theory
  public void checkSourceAddedOnRenameSide_dirtyWorkTree_failed(MergeStrategy strategy, boolean isRenameInOurs) throws Exception {
    Git git = Git.wrap(db);
    String originalContent = "a\nb\nc";
    String slightlyModifiedContent = "a\nb\nb";
    String originalFilename = "file2";
    String renameFilename = "file1";
    Map<String, String> filesWithRename = Map.of(renameFilename, originalContent, originalFilename, "Added content");
    Map<String, String> filesWithModify = Map.of(originalFilename, slightlyModifiedContent);
    RevCommit mergeCommit = setUpMerge(git, Map.of(originalFilename, originalContent), isRenameInOurs? filesWithRename: filesWithModify, isRenameInOurs? filesWithModify: filesWithRename);
    ...
    MergeResult mergeResult = git.merge().include(mergeCommit).setStrategy(strategy).call();
    assertEquals(mergeResult.getMergeStatus(), MergeStatus.FAILED);
    assertEquals(mergeResult.getFailingPaths(), Map.of(originalFilename, DIRTY_WORKTREE));
    // Nothing was overwritten
    assertEquals(read(originalFilename), "Modified content 1");
  }

  /**
   *  Same as Testcase 3.1, with dirty index preventing the file from being added.
   */
  @Theory
  public void checkSourceAddedOnRenameSide_dirtyIndex_failed(MergeStrategy strategy, boolean isRenameInOurs) throws Exception {
    Git git = Git.wrap(db);
    String originalContent = "a\nb\nc";
    String slightlyModifiedContent = "a\nb\nb";
    String originalFilename = "file2";
    ...
    MergeResult mergeResult = git.merge().include(mergeCommit).setStrategy(strategy).call();
    assertEquals(mergeResult.getMergeStatus(), MergeStatus.FAILED);
    assertEquals(mergeResult.getFailingPaths(), Map.of(originalFilename, DIRTY_INDEX));
    // Nothing was overwritten
    assertEquals(read(originalFilename), "Modified content 1");
  }
  ...
  @Theory
  public void checkRenameDir_allFilesMoved_someFilesAddedOnRenameSide(MergeStrategy strategy, @FromDataPoints("renameLists") List<Entry<String, String>> renamePairs, boolean isRenameInOurs) throws Exception {
    String originalContent = "a\nb\nc";
    String slightlyModifiedContent = "a\nb\nb";
    Map<String, String> originalFiles = new HashMap<>();
    for (Entry<String, String> renamePair : renamePairs) {
  ...
  @Theory
  public void checkRenameDir_allFilesMoved_someFilesAddedOnModifySide(MergeStrategy strategy, @FromDataPoints("renameLists") List<Entry<String, String>> renamePairs, boolean isRenameInOurs) throws Exception {
    String originalContent = "a\nb\nc";
    String slightlyModifiedContent = "a\nb\nb";
    Map<String, String> originalFiles = new HashMap<>();
    for (Entry<String, String> renamePair : renamePairs) {
  ...
  @Theory
  public void checkRenameDir_allFilesMoved_someFilesAddedOnBothSides(MergeStrategy strategy, @FromDataPoints("renameLists") List<Entry<String, String>> renamePairs, boolean isRenameInOurs) throws Exception {
    String originalContent = "a\nb\nc";
    String slightlyModifiedContent = "a\nb\nb";
    Map<String, String> originalFiles = new HashMap<>();
    for (Entry<String, String> renamePair : renamePairs) {
    ...
    Map<String, String> expectedFiles = new HashMap<>();
    // This should all be under rename dir, when directory renames is implemented
    for (Entry<String, String> renamePair : renamePairs) {
      expectedFiles.put(renamePair.getValue(), slightlyModifiedContent + renamePair.getKey());
    }
    for (String fileAddition : fileAdditionsData) {
