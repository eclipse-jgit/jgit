...
/**
 * Tests for {@link ResolveMerger} handling of renames.
 *
 * <p>
 ...
 *   B: test/file1 X_1
 *   O: test/file1 X_2
 *   T: test/file2 X_1, file3 Y
 * </pre>
 * <p>
 * means base commit B contains test/file1 with content X_1.
 * <p>
 * X_2 is the content, modified form X_1, Y is entirely unrelated to X_1(2)
 * content.
 *
 * <p>
 * All the test DataPoints below contain the mappings source (original) File ->
 * target (rename) File
 */
...
public class MergerRenameTest extends RepositoryTestCase {
	...
	public void testRename_merged(MergeStrategy strategy, String originalName,
			String originalContent, String oursName, String oursContent,
			String theirsName, String theirsContent,
			Map<String, String> expectedFileContents) throws Exception {
		...
		testRename_merged(strategy, originalFiles, oursFiles, theirsFiles,
				expectedFileContents);
	}

	public void testRename_merged(MergeStrategy strategy,
			Map<String, String> originalFilesToContents,
			Map<String, String> oursFilesToContents,
			Map<String, String> theirsFilesToContents,
			Map<String, String> expectedFileContents) throws Exception {
		if (!strategy.equals(MergeStrategy.RECURSIVE)) {
			return;
		}
		MergeResult mergeResult = mergeRename(strategy, originalFilesToContents,
				oursFilesToContents, theirsFilesToContents);
		assertEquals(mergeResult.getMergeStatus(), MergeStatus.MERGED);
		Set<String> expectedIndexContent = new HashSet<>();
		for (Entry<String, String> expectedFile : expectedFileContents
				.entrySet()) {
			if (expectedFile.getValue() != null) {
				...
				expectedIndexContent.add(String.format(
						"%s, mode:100644, content:%s", expectedFile.getKey(),
	...
	public void testRename_withConflict(MergeStrategy strategy,
		...
		for (Entry<String, String> expectedFile : expectedFileContents
			...
			if (expectedFile.getValue() == null) {
				...
				// assertEquals(read(expectedFile.getKey()),
				// expectedFile.getValue());
			} else {
				assertEquals(expectedFile.getValue(),
						read(expectedFile.getKey()));
	...
	public void testRename_withConflict(MergeStrategy strategy,
			...
			Map<String, String> theirsFilesToContents,
			Map<String, String> expectedFileContents,
			Set<String> expectedConflicts, Set<String> expectedIndexContent)
			throws Exception {

		testRename_withConflict(strategy, originalFilesToContents,
				oursFilesToContents, theirsFilesToContents,
				expectedFileContents, expectedConflicts);
	...
	private RevCommit setUpMerge(Git git,
			Map<String, String> originalFilesToContents,
			Map<String, String> oursFilesToContents,
			Map<String, String> theirsFilesToContents) throws Exception {
		// master
		for (Entry<String, String> originalFile : originalFilesToContents
				.entrySet()) {
		...
		for (Entry<String, String> theirsFile : theirsFilesToContents
				.entrySet()) {
	...
	private MergeResult mergeRename(MergeStrategy strategy,
			...
			Map<String, String> theirsFilesToContents) throws Exception {
		Git git = Git.wrap(db);
		RevCommit mergeCommit = setUpMerge(git, originalFilesToContents,
				oursFilesToContents, theirsFilesToContents);
	...
	public static final List<List<Entry<String, String>>> renameListsData = List
			.of(
	 ...
	 *
	 * <p>
	 * Test classic conflict, one side renamed, other modified.
	 * <p>
	 * This should be merged cleanly.
	 *
	 * <p>
	 * Examines various source -> target rename scenarios that have different
	 * sorting in TreeWalk
	 * <p>
	 * E.g.:
	 * 
	...
	public void testRenameModify_merged(MergeStrategy strategy,
			List<Entry<String, String>> renamePairs, boolean isRenameInOurs)
			throws Exception {
	...
	public void checkRenameModifyFile_merged(MergeStrategy strategy,
			@FromDataPoints("renameLists") List<Entry<String, String>> renamePairs,
	 ...
	 *
	 * <p>
	 * TODO: extend when dir rename detection is implemented
	 */
	@DataPoints("renameListsSplit")
	public static final List<List<Entry<String, String>>> renameListsSplitData = List
			.of(
					...
					// Some files moved to subdir, other remain in the original
					// folder
					List.of(Map.entry("test/a/file1", "test/a/file1"),
							Map.entry("test/a/file2", "test/a/w/file2")),
					// Some files moved to parent dir, other remain in the
					// original folder
					List.of(Map.entry("test/a/file1", "test/a/file1"),
	...
	public void checkRenameSplitDir_merged(MergeStrategy strategy,
			@FromDataPoints("renameListsSplit") List<Entry<String, String>> renamePairs,
			boolean isRenameInOurs) throws Exception {
		testRenameModify_merged(strategy, renamePairs, isRenameInOurs);
	 ...
	 *
	 * <p>
	 * Classic rename, with one side renamed and various modifications on either
	 * sides.
	 * <p>
	 * This should be merged cleanly, regardless of modifications on either
	 * side, if content merge does not have conflicts.
	 * 
	 * <pre>
	 *   B: test/file1 X_1
	 *   O: test/file1 X_1(2)
	...
	public void checkRenameModify_merged(MergeStrategy strategy,
			boolean isRenameInOurs, boolean modifyInRename,
	 ...
	 *
	 * <p>
	 * Classic rename, with one side renamed and modifications, causing content
	 * merge conflict.
	 * <p>
	 * TODO: similar test is needed for git link merging, mode conflicts and
	 * file/dir conflicts.
	 *
	 * <pre>
	 *   B: test/file1 X_1
	 ...
	 * </pre>
	 * <p>
	 * The rename conflict was resolved, but there is a merge conflict on the
	 * content.
	 * <p>
	 * Since rename was detected, index/work tree do not contain the original
	 * file
	 */
	@Theory
	public void checkRenameModify_conflict(MergeStrategy strategy,
		...
		expectedIndex.add(String.format("%s, mode:100644, stage:1, content:%s",
				renameFilename, originalContent));
	 ...
	 *
	 * <p>
	 * Same as Testcase 1.2, but both sides renamed to the same name
	 * 
	 * <pre>
	 *   B: test/file1 X_1
	 *   O: test/file2 X_1(2)
	 ...
	 *
	 * <p>
	 * Same as Testcase 1.3, but both sides renamed to the same name
	 * 
	 * <pre>
	 *  B: test/file1 X_1
	 *  O: test/file2 X(2)
	...
	public void checkRenameBothModify_conflict(MergeStrategy strategy)
		...
		testRename_withConflict(strategy, originalFiles, oursFiles, theirsFiles,
				expectedFiles, expectedConflicts, expectedIndex);
	}

	/**
	 * Testcase 1.6
	 * <p>
	 * Cross-side renames are no supported. Both files are present in the merge
	 * commit.
	 * 
	 * <pre>
	 *    B: test/file1 X
	 ...
	 * Testcase 1.7
	 * <p>
	 * File was completely replaced on non-rename side.
	 * <p>
	 * Since we do not compute cross-side similarity, this does not result in
	 * rename/delete conflict.
	 * <p>
	 * The rename is detected and the files are content-merged.
	 * <p>
	 * This might seem obvious, however see Testcase 3.5 why this behaviour can
	 * be problematic.
	 * <p>
	 * Note: Without rename detection, we would have modify/delete conflict.
	 * 
	 * <pre>
	 *    B: test/file1 X_1
	 *    O: test/file1 Y
	 ...
	 * Testcase 1.8
	 * <p>
	 * Same as above, but the rename side modified the content, which results in
	 * content conflict.
	 * 
	 * <pre>
	 *    B: test/file1 X_1
	...
	public void checkRenameWithReplace_conflict(MergeStrategy strategy,
		...
		expectedIndex.add(String.format("%s, mode:100644, stage:%s, content:%s",
				renameFilename, renameStage, slightlyModifiedContent));
		expectedIndex.add(String.format("%s, mode:100644, stage:%s, content:%s",
				renameFilename, modifyStage, "Unrelated content"));
	 ...
	 *
	 * <p>
	 * This section covers various rename conflicts
	 */

	/**
	 * Testcase 2.1
	 * <p>
	 * Renamed on both sides to different names. Conflict with all files present
	 * in the index.
	 * 
	 * <pre>
	 *    B: test/file1 X_1
	...
	public void checkRenameRename_conflict(MergeStrategy strategy,
			boolean modifyInOurs, boolean modifyInTheirs) throws Exception {
		String originalFilename = "test/file1";
		String originalContent = "a\nb\nc";
		String slightlyModifiedContent = "a\nb\nb";
		String oursFilename = "test/file2";
		...
		Set<String> expectedIndex = new HashSet<>();
		expectedIndex.add(String.format("%s, mode:100644, stage:1, content:%s",
				originalFilename, originalContent));
		expectedIndex.add(String.format("%s, mode:100644, stage:2, content:%s",
				oursFilename, oursContent));
		expectedIndex.add(String.format("%s, mode:100644, stage:3, content:%s",
				theirsFilename, theirsContent));
		testRename_withConflict(strategy, originalFiles, oursFiles, theirsFiles,
				expectedFiles, expectedConflicts, expectedIndex);
	}
	 ...
	 * </pre>
	 * <p>
	 * Note: index is always populated with the contents of the rename file,
	 * regardless of the pull side
	 */
	@Theory
	public void checkRenameDelete_conflict(MergeStrategy strategy,
			boolean isRenameInOurs, boolean shouldModify) throws Exception {
		String originalFilename = "test/file1";
		String originalContent = "a\nb\nc";
		String slightlyModifiedContent = "a\nb\nb";
		String renameFilename = "test/file2";
	...
	public void checkDelete_conflict(MergeStrategy strategy,
		...
		expectedIndex.add(String.format("%s, mode:100644, stage:1, content:%s",
				originalFilename, originalContent));
		expectedIndex.add(String.format("%s, mode:100644, stage:%s, content:%s",
				originalFilename, renameStage, slightlyModifiedContent));
	 ...
	 * Testcase 2.3
	 * <p>
	 * Related to Testcase 1.7, 1.8
	 * <p>
	 * Rename/Add conflict: one side added the file that the other side renamed
	 * to.
	 * <p>
	 * Rename processing is switched off for this path, which results in content
	 * conflict on target file and delete/modify on source file.
	 * <p>
	 * NOTE: This is handled differently in c-git, see below.
	 * 
	 * <pre>
	 *  B: test/file1 X_1
	 ...
	 * </pre>
	 * 
	 * TODO: c-git creates an X_1 vs X_2 merge which is than merged with Y
	 * (results in nested conflict markers), and only reports file2 as unmerged.
	 */
	@Theory
	public void checkRenameAddCollision_conflict(MergeStrategy strategy,
			boolean isRenameInOurs, boolean shouldModifyRename,
		...
		if (shouldModifyOriginal) {
			expectedIndex
					.add(String.format("%s, mode:100644, stage:1, content:%s",
							originalFilename, originalContent));
			expectedIndex.add(String.format(
					"%s, mode:100644, stage:%s, content:%s", originalFilename,
					modifyStage, filesWithModify.get(originalFilename)));
		}
		expectedIndex.add(String.format("%s, mode:100644, stage:%s, content:%s",
				renameFilename, modifyStage, "Unrelated file"));
		expectedIndex.add(String.format("%s, mode:100644, stage:%s, content:%s",
				renameFilename, renameStage,
		...
		String theirsContent = isRenameInOurs ? "Unrelated file"
				: filesWithRename.get(renameFilename);
		expectedFiles.put(renameFilename, String.format(
	 ...
	 * Related to Testcase 1.7, 1.8
	 * <p>
	 * The target rename exists in the base and is kept by one of the sides.
	 * <p>
	 * Since it was not touched, and other side deleted it, the file is deleted.
	 * 
	 * <pre>
	...
	public void checkRenameModifyCollision_collisionUnchanged_merged(
			MergeStrategy strategy, boolean isRenameInOurs,
			boolean shouldModifyRename, boolean shouldModifyOriginal)
			throws Exception {
		String originalFilename = "test/file1";
		String originalContent = "a\nb\nc";
		String slightlyModifiedContent = "a\nb\nb";
	 ...
	 * Testcase 2.5
	 * <p>
	 * Same as Testcase 2.4, but the side that kept the target also modified it.
	 * <p>
	 * Makes it identical to Testcase 2.3.
	 * 
	 * <pre>
	 *   B: test/file1 X_1 test/file2 Y
	...
	public void checkRenameModifyCollision_collisionUnchanged_conflicting(
		...
		expectedIndex.add(String.format("%s, mode:100644, stage:1, content:%s",
				renameFilename, "Original unrelated file"));
		expectedIndex.add(String.format("%s, mode:100644, stage:%s, content:%s",
				renameFilename, modifyStage, "Unrelated file"));
		expectedIndex.add(String.format("%s, mode:100644, stage:%s, content:%s",
				renameFilename, renameStage,
	...
	/**
	 * Testcase 2.6 Same as Testcase 2.3, but the side with add collision also
	 * removed the source file. Since source file is missing on both sides, it
	 * is not present on merge, not reported as a conflict.
	 * 
	 * <pre>
	 *    B: test/file1 X_1
	 *    O: test/file2 Y
	 ...
	 * Testcase 2.7
	 * <p>
	 * Both sides added the file but under different names, with one side
	 * colliding with the rename.
	 * <p>
	 * Because of add/rename collision, the rename detection is off which
	 * results in content merge resolution.
	 * 
	 * <pre>
	...
	public void checkFilesNamesSwappedOnSides_conflict(MergeStrategy strategy,
		...
		Map<String, String> oursFiles = isRenameInOurs ? filesWithRename
				: filesWithModify;
		Map<String, String> theirsFiles = isRenameInOurs ? filesWithModify
				: filesWithRename;
		Set<String> expectedConflicts = new HashSet<>();
		expectedConflicts.add(renameFilename);
		if (shouldModifyOriginal) {
		...
		if (shouldModifyOriginal) {
			...
			expectedIndex.add(String.format(
					"%s, mode:100644, stage:%s, content:%s", originalFilename,
					modifyStage, shouldModifyOriginal ? slightlyModifiedContent
							: originalContent));
			expectedIndex
					.add(String.format("%s, mode:100644, stage:%s, content:%s",
							originalFilename, renameStage, "Unrelated file"));
		} else {
			expectedIndex.add(String.format("%s, mode:100644, content:%s",
					originalFilename, "Unrelated file"));
		}
		expectedIndex.add(String.format("%s, mode:100644, stage:1, content:%s",
				renameFilename, "Unrelated base content"));
		expectedIndex.add(String.format("%s, mode:100644, stage:%s, content:%s",
				renameFilename, modifyStage, "Unrelated file"));
		expectedIndex.add(String.format("%s, mode:100644, stage:%s, content:%s",
				renameFilename, renameStage,
	 ...
	 * test/file3 A_1 vs X_1 content merge conflict.
	 */
	@Theory
	public void checkRenameSameTarget_differentSource_conflict(
			MergeStrategy strategy, boolean keepOriginalContent)
			throws Exception {
		...
		Map<String, String> oursFiles = new HashMap<>();
		oursFiles.put(renameFilename, keepOriginalContent ? originalContent1
				: slightlyModifiedContent1);
		Map<String, String> theirsFiles = new HashMap<>();
		theirsFiles.put(renameFilename, keepOriginalContent ? originalContent2
				: slightlyModifiedContent2);
		Set<String> expectedConflicts = new HashSet<>();
		expectedConflicts.add(renameFilename);
		Set<String> expectedIndex = new HashSet<>();
		expectedIndex.add(String.format("%s, mode:100644, stage:2, content:%s",
				renameFilename, oursFiles.get(renameFilename)));
		expectedIndex.add(String.format("%s, mode:100644, stage:3, content:%s",
				renameFilename, theirsFiles.get(renameFilename)));
		Map<String, String> expectedFiles = new HashMap<>();
	 ...
	 *
	 * <p>
	 * This section covers edge-cases related to file name correlation
	 */

	/**
	 * Testcase 3.1
	 * <p>
	 * This test checks that the files, are involved as a source of rename are
	 * not accidentally dropped,and it does not result in collision.
	 * 
	 * <pre>
	 *    B: test/file1 X_1
	 *    O: test/file1 X_2
	 ...
	 * Testcase 3.2
	 * <p>
	 * Same as Testcase 3.1, but the source was renamed on both sides.
	 * 
	 * <pre>
	 *    B: test/file1 X_1
	 *    O: test/file2 X_2
	...
	public void checkSourceAdded_renameOnBothSides_merged(
			MergeStrategy strategy, boolean isAddInOurs, boolean shouldModify)
			throws Exception {
		String originalFilename = "test/file1";
		String originalContent = "a\nb\nc";
		String slightlyModifiedContent = "a\nb\nb";
	 ...
	 * Testcase 3.3
	 * <p>
	 * Same as Testcase 3.2, but the content merge on added file is required.
	 * 
	 * <pre>
	 *    B: test/file1 X_1
	...
	public void checkSourceAddedOnBothSides_renameOnBothSides_merged(
			MergeStrategy strategy, boolean shouldModify) throws Exception {
		String originalFilename = "test/file1";
		String originalContent = "a\nb\nc";
		String slightlyModifiedContent = "a\nb\nb";
		...
		expectedFiles.put(renameFilename,
				shouldModify ? slightlyModifiedContent : originalContent);
		expectedFiles.put(originalFilename, String.format(
				"<<<<<<< HEAD\n%s\n=======\n%s", mergeContent1, mergeContent2));
		Set<String> expectedConflicts = new HashSet<>();
		expectedConflicts.add(originalFilename);
		testRename_withConflict(strategy, originalFiles, oursFiles, theirsFiles,
				expectedFiles, expectedConflicts);
	}

	/**
	 * Testcase 3.4
	 * <p>
	 * Ideally, this should be detected as rename/delete conflict, but since we
	 * do not compute cross-side similarity, this just results in content merge.
	 * <p>
	 * This might not be a problem, however, see Testcase 3.5.
	 * 
	 * <pre>
	 *    B: test/file1 X_1
	 *    O: test/file1 Y
	 ...
	 * </pre>
	 * <p>
	 * With the current implementation, we always assume the non-rename side
	 * modified the file, while it is possible it completely replaced it.
	 * <p>
	 * Detecting it would require either:
	 * <p>
	 * - content test/file1 OvsT content comparison, since they do not have a
	 * common matching base.
	 * <p>
	 * - similarity {@code org.eclipse.jgit.diff.DiffEntry#score} calculations
	 * for {@code ChangeType#MODIFY}
	 */
	@Theory
	public void checkSourceAddedOnBothSides_withModification_conflict(
			MergeStrategy strategy, boolean isRenameInOurs) throws Exception {
		String originalFilename = "test/file1";
		String originalContent = "a\nb\nc";
		String slightlyModifiedContent = "a\nb\nb";
		String renameFilename = "test/file2";
	 ...
	 * Testcase 3.5
	 * <p>
	 * See Testcase 3.4. The results of this merge are meaningless.
	 * <p>
	 * Since rename/delete conflict was not detected, the unrelated file was
	 * chosen as a content for rename file, since the rename file was not
	 * modified comparing to base.
	 * 
	 * <pre>
	 *    B: test/file1 X_1
	 *    O: test/file1 Y
	 ...
	 * </pre>
	 * <p>
	 * Note: without the rename detection, this would be content merged as
	 ...
	 * Testcase 3.6
	 * <p>
	 * It does not matter that test/file2 Z is present in base. It is missing in
	 * both commits, so missing in merge.
	 * 
	 * <pre>
	 *   B: test/file1 X_1 test/file2 Z
	 *   O: test/file1 X_2
	...
	public void checkRenameModify_withRenameExistedInBase_merged(
			MergeStrategy strategy, boolean isRenameInOurs,
			boolean shouldModifyOurs, boolean shouldModifyTheirs)
			throws Exception {
		String originalFilename = "test/file1";
		String originalContent = "a\nb\nc";
		String slightlyModifiedContent = "a\nb\nb";
	 ...
	 * Testcase 3.7
	 * <p>
	 * Same as Testcase 3.7, but renamed on both sides.
	 * 
	 * <pre>
	 *   B: test/file1 X_1 test/file2 Z
	 *   O: test/file2 X_2
	...
	public void checkRenameBoth_withRenameExistedInBase_merged(
			MergeStrategy strategy, boolean shouldModifyOurs,
	 ...
	 * Testcase 3.8
	 * <p>
	 * Rename/delete conflict, but index is updated with the added file.
	 * 
	 * <pre>
	 *    B: test/file1 X_1
	 *    O:
	...
	public void checkMultipleRenames_correlationOnSource_renameModify_renameDelete_conflict(
			MergeStrategy strategy, boolean isSwapInOurs, boolean shouldModify,
	...
	public void checkMultipleRenames_swapRename_merged(MergeStrategy strategy,
			boolean shouldModifyRename1, boolean shouldModifyRename2)
			throws Exception {
		...
		expectedFiles.put(renameFilename1,
				shouldModifyRename2 ? slightlyModifiedContent2
						: originalContent2);
		testRename_merged(strategy, originalFiles, oursFilesWithRename,
				theirsFilesWithRename, expectedFiles);
	}

	/**
	 * Testcase 3.11
	 * <p>
	 * Same as Testcase 2.6, but the side keeps the rename files of opposite
	 * side.
	 * 
	 * <pre>
	 *    B: test/file1 A_1, test/file2 X_1
	...
	public void checkRenameSameTarget_differentSource_wihSourcePresentOnSides_conflict(
		...
		oursFiles.put(renameFilename, keepOriginalContent ? originalContent1
				: slightlyModifiedContent1);
		oursFiles.put(originalFilename2, keepOriginalContent ? originalContent2
				: slightlyModifiedContent2);
		Map<String, String> theirsFiles = new HashMap<>();
		theirsFiles.put(renameFilename, keepOriginalContent ? originalContent2
				: slightlyModifiedContent2);
		...
		if (!keepOriginalContent) {
			expectedIndex
					.add(String.format("%s, mode:100644, stage:1, content:%s",
							originalFilename1, originalContent1));
			expectedIndex
					.add(String.format("%s, mode:100644, stage:1, content:%s",
							originalFilename2, originalContent2));
			expectedIndex.add(String.format(
					"%s, mode:100644, stage:2, content:%s", originalFilename2,
					oursFiles.get(originalFilename2)));
			expectedIndex.add(String.format(
					"%s, mode:100644, stage:3, content:%s", originalFilename1,
	 ...
	 * Testcase 3.12
	 * <p>
	 * See Testcase 2.6. The conflicting rename is switched off, but the
	 * correlating rename is not affected.
	 * 
	 * <pre>
	...
	public void checkMultipleRenames_renameSameTarget_differentSource_conflict(
		...
		Set<String> expectedIndex = new HashSet<>();
		expectedIndex.add(String.format("%s, mode:100644, content:%s",
				originalFilename1, correlatingRenameContent));
		expectedIndex.add(String.format("%s, mode:100644, stage:2, content:%s",
				renameFilename, oursFiles.get(renameFilename)));
		expectedIndex.add(String.format("%s, mode:100644, stage:3, content:%s",
				renameFilename, theirsFiles.get(renameFilename)));
	 ...
	 * Testcase 3.13
	 * <p>
	 * See Testcase 2.1, but also added file with source name.
	 * <p>
	 * This conflict would be extremely hard to represent in index, hence we
	 * have to resolve it.
	 * 
	 * <pre>
	 *    B: test/file1 X_1
	...
	public void checkRenameRename_withSourceAdded_conflict(
			MergeStrategy strategy, boolean addInOurs) throws Exception {
		String originalFilename = "test/file1";
		String originalContent = "a\nb\nc";
		String slightlyModifiedContent = "a\nb\nb";
		...
		testRename_merged(strategy, originalFiles,
				...
				addInOurs ? filesWithRename : filesWithAdd, expectedFiles);
	}

	/**
	 * Testcase 3.14
	 * <p>
	 * Same as 3.13, but source is present on both sides.
	 * <p>
	 * Since we can not represent it in index without dropping the source,
	 * switch off the rename.
	 * 
	 * <pre>
	 *    B: test/file1 X_1
	 *    O: test/file2 X_2, test/file1 Z
	...
	public void checkRenameRename_withSourceAddedOnBothSides_merged(
		...
		testRename_merged(strategy, originalFiles, oursFiles, theirsFile,
				expectedFiles);
	}
	 ...
	 * Testcase 3.15
	 * <p>
	 * See Testcase 2.1, but also added file with target name.
	 * <p>
	 * Since we can not represent it in index without dropping the source,
	 * switch off the rename.
	 * 
	...
	public void checkRenameRename_withRenameAddCollision_conflicting(
			MergeStrategy strategy, boolean isAddInOurs) throws Exception {
		String originalFilename = "test/file1";
		String originalContent = "a\nb\nc";
		String slightlyModifiedContent1 = "a\nb\nb";
		String slightlyModifiedContent2 = "a\nb\nc\nd";
		...
		int renameStage = isAddInOurs ? DirCacheEntry.STAGE_3
				: DirCacheEntry.STAGE_2;
		expectedIndex.add(String.format("%s, mode:100644, content:%s",
				renameFilename1, slightlyModifiedContent1));
		expectedIndex.add(String.format("%s, mode:100644, stage:%s, content:%s",
				renameFilename2, modifyStage, "Unrelated file"));
		expectedIndex.add(String.format("%s, mode:100644, stage:%s, content:%s",
				renameFilename2, renameStage, slightlyModifiedContent2));
		...
		String theirsContent = isAddInOurs ? slightlyModifiedContent2
				: "Unrelated file";
		expectedFiles.put(renameFilename2, String.format(
				"<<<<<<< HEAD\n%s\n=======\n%s", oursContent, theirsContent));
		testRename_withConflict(strategy, originalFiles,
				isAddInOurs ? filesWithAdd : filesWithRename,
				isAddInOurs ? filesWithRename : filesWithAdd, expectedFiles,
	 ...
	 * Testcase 3.16
	 * <p>
	 * Same as Testcase 3.15, but with rename/modify collision.
	 * 
	 * <pre>
	...
	public void checkRenameRename_withRenameModifyCollision_conflicting(
		...
		expectedIndex.add(String.format("%s, mode:100644, content:%s",
				renameFilename1, slightlyModifiedContent1));
		expectedIndex.add(String.format("%s, mode:100644, stage:1, content:%s",
				renameFilename2, "Base unrelated file"));
		expectedIndex.add(String.format("%s, mode:100644, stage:%s, content:%s",
				renameFilename2, modifyStage, "Unrelated file"));
		expectedIndex.add(String.format("%s, mode:100644, stage:%s, content:%s",
				renameFilename2, renameStage, slightlyModifiedContent2));
	 ...
	 * Testcase 3.17
	 * <p>
	 * Same as Testcase 3.15, but the collision was not modified.
	 * <p>
	 * Since it is untouched in one side and deleted in other, it is not present
	 * on merge and the regular rename/rename conflict is reported.
	 * 
	 * <pre>
	 *    B: test/file1 X_1, test/file3 Z
	 *    O: test/file2 X_2, test/file3 Z
	...
	public void checkRenameRename_withRenameCollision_conflicting(
		...
		expectedIndex.add(String.format("%s, mode:100644, stage:1, content:%s",
				originalFilename, originalContent));
		expectedIndex.add(String.format("%s, mode:100644, stage:%s, content:%s",
				renameFilename1, modifyStage, slightlyModifiedContent1));
		expectedIndex.add(String.format("%s, mode:100644, stage:%s, content:%s",
				renameFilename2, renameStage, slightlyModifiedContent2));
	 ...
	 * SECTION 4
	 * <p>
	 * Tests the behaviour for dirty index and working tree
	 */

	@Theory
	public void checkRenameRename_dirtyIndex_fail(MergeStrategy strategy,
			boolean dirtyOriginal, boolean dirtyOurs, boolean dirtyTheirs)
			throws Exception {
		...
		RevCommit mergeCommit = setUpMerge(git,
				Map.of(originalFilename, content),
				Map.of(renameFilename1, content),
				Map.of(renameFilename2, content));

		String failingPath = null;
		if (dirtyTheirs) {
			writeTrashFile(renameFilename2, "Modified content 1");
			git.add().addFilepattern(renameFilename2).call();
			failingPath = renameFilename2;
		}
		if (dirtyOurs) {
			writeTrashFile(renameFilename1, "Modified content 2");
			git.add().addFilepattern(renameFilename1).call();
			failingPath = renameFilename1;
		}
		if (dirtyOriginal) {
			writeTrashFile(originalFilename, "Modified content 3");
		...
		assertEquals(mergeResult.getMergeStatus(), MergeStatus.FAILED);

		assertEquals(mergeResult.getFailingPaths(),
				Map.of(failingPath, DIRTY_INDEX));
		// Nothing was overwritten
		if (dirtyOriginal) {
			assertEquals(read(originalFilename), "Modified content 3");
		}
		if (dirtyOurs) {
			assertEquals(read(renameFilename1), "Modified content 2");
		}
		if (dirtyTheirs) {
			assertEquals(read(renameFilename2), "Modified content 1");
		}
	}
	...
	public void checkRenameRename_dirtyWorkingTree_fail(MergeStrategy strategy,
		...
		if (dirtyOurs || dirtyTheirs) {
			assertEquals(mergeResult.getMergeStatus(), MergeStatus.FAILED);
			assertEquals(mergeResult.getFailingPaths(),
					Map.of(failingPath, DIRTY_WORKTREE));
		} else {
			// if unrelated file was modified in working tree, it won't be
			// overwritten by merge,
			// so allowed to be modified locally
			assertEquals(mergeResult.getMergeStatus(), MergeStatus.CONFLICTING);
		}
		// Nothing was overwritten
		if (dirtyOriginal) {
	...
	public void checkRenameModify_dirtyIndex_fail(MergeStrategy strategy,
			boolean isRenameInOurs, boolean dirtyOriginal, boolean dirtyRename)
			throws Exception {
		...
		Map<String, String> filesWithRename = Map.of(renameFilename,
				originalContent);
		Map<String, String> filesWithModify = Map.of(originalFilename,
				slightlyModifiedContent);
		RevCommit mergeCommit = setUpMerge(git,
				Map.of(originalFilename, originalContent),
				isRenameInOurs ? filesWithRename : filesWithModify,
				isRenameInOurs ? filesWithModify : filesWithRename);

		String failingPath = null;
		if (dirtyOriginal) {
			writeTrashFile(originalFilename, "Modified content 1");
			git.add().addFilepattern(originalFilename).call();
			failingPath = originalFilename;
		}
		if (dirtyRename) {
			writeTrashFile(renameFilename, "Modified content 2");
		...
		// Nothing was overwritten
		if (dirtyOriginal) {
			assertEquals(read(originalFilename), "Modified content 1");
		}
		if (dirtyRename) {
			assertEquals(read(renameFilename), "Modified content 2");
		}
	}
	...
	public void checkRenameModify_dirtyWorkingTree_renameInTheirs_fail(
			MergeStrategy strategy, boolean isRenameInOurs,
			boolean dirtyOriginal, boolean dirtyRename) throws Exception {
		if (!dirtyOriginal && !dirtyRename) {
			return;
		}
	...
	public void checkRenameModify_dirtyWorkingTree_renameInOurs_merged(
		...
		Map<String, String> filesWithModify = Map.of(originalFilename,
				slightlyModifiedContent);
		RevCommit mergeCommit = setUpMerge(git,
				Map.of(originalFilename, originalContent), filesWithRename,
				filesWithModify);
		...
		MergeResult mergeResult = git.merge().include(mergeCommit)
				.setStrategy(strategy).call();
		assertEquals(mergeResult.getMergeStatus(), MergeStatus.MERGED);
		// This file is just the regular local untracked file, not involved into
		// merge
		// The new content is written to renameFilename and there is no need to
		// delete originalFilename
		// since it is not present in oursCommit
		assertEquals(read(originalFilename), "Modified content 1");
		assertEquals(read(renameFilename), slightlyModifiedContent);
	}

	/**
	 * Same as Testcase 3.1, with dirty work tree preventing the file from being
	 * added.
	 */
	@Theory
	public void checkSourceAddedOnRenameSide_dirtyWorkTree_failed(
		...
		String renameFilename = "file1";
		Map<String, String> filesWithRename = Map.of(renameFilename,
				originalContent, originalFilename, "Added content");
		Map<String, String> filesWithModify = Map.of(originalFilename,
				slightlyModifiedContent);
		RevCommit mergeCommit = setUpMerge(git,
				Map.of(originalFilename, originalContent),
		...
		assertEquals(mergeResult.getMergeStatus(), MergeStatus.FAILED);
		assertEquals(mergeResult.getFailingPaths(),
				Map.of(originalFilename, DIRTY_WORKTREE));
		// Nothing was overwritten
		assertEquals(read(originalFilename), "Modified content 1");
	}

	/**
	 * Same as Testcase 3.1, with dirty index preventing the file from being
	 * added.
	 */
	@Theory
	public void checkSourceAddedOnRenameSide_dirtyIndex_failed(
		...
		assertEquals(mergeResult.getMergeStatus(), MergeStatus.FAILED);
		assertEquals(mergeResult.getFailingPaths(),
				Map.of(originalFilename, DIRTY_INDEX));
		// Nothing was overwritten
		assertEquals(read(originalFilename), "Modified content 1");
	}
	 ...
	 * SECTION 5
	 * <p>
	 * Same as Testcase 1.1, but also tests that files, adjacent to rename are
	 * not dropped.
	 */

	/**
	 * Various files, that are present in the directories, containing the
	 * renames.
	 * <p>
	 * This test cases make sure files are not disappearing on rename detection
	 *
	 * <p>
	 * TODO: extend when directory rename detection is implemented
	 */
	@DataPoints("fileAdditions")
	public static final List<String> fileAdditionsData = List.of("file1.1",
			"file3", "file0", "a/file", "z/file", "a/sub/file", "z/sub/file");

	/**
	 * Single file renames
	 */
	@DataPoints("singleRenamePairs")
	public static final List<Entry<String, String>> singleRenamePairsData = List
			.of(
					// Direct rename of file
	 ...
	 * Testcase 5.1
	 * <p>
	 * Note: this test does not take into account directory renames, since it is
	 * not implemented (see Testcase 2.2, Testcase 2.3).
	 * <p>
	 * Examples:
	 * 
	 * <pre>
	...
	public void checkRenameModify_withFilesInSameDir_merged(
			...
			@FromDataPoints("singleRenamePairs") Entry<String, String> renamePair,
	...
	/**
	 * Testcase 5.2 Explicit test that directory renames are not detected,
	...
	public void checkRenameDir_allFilesMoved_someFilesAddedOnBothSides(
		...
		Map<String, String> expectedFiles = new HashMap<>();
		// This should all be under rename dir, when directory renames is
		// implemented
		for (Entry<String, String> renamePair : renamePairs) {
			expectedFiles.put(renamePair.getValue(),
					slightlyModifiedContent + renamePair.getKey());
