...
public class BlameGeneratorCacheTest extends RepositoryTestCase {
	...
	public void blame_movingBlock_correctRegions() throws Exception {
		RevCommit c1, c2, c3;
		try (TestRepository<FileRepository> r = new TestRepository<>(db)) {
			c1 = commit(r, lines("first", "last"));
			c2 = commit(r, lines("first", "middle", "last"), c1);
			c3 = commit(r, lines("first", "middle", "extra", "last"), c2);
		}

		List<EmittedRegion> expectedRegions = Arrays.asList(
				/* first */ new EmittedRegion(c1, 0, 1),
				/* middle */ new EmittedRegion(c2, 1, 2),
				/* extra */ new EmittedRegion(c3, 2, 3),
				/* last */ new EmittedRegion(c1, 3, 4));
