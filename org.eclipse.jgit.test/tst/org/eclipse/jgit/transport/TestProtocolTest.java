...
public class TestProtocolTest {
  ...
  @Before
	...
	public void fetchReportsLockFailureWhenAutoFollowTagIsCreated()
		...
		URIish uri = newRepositoryTestConnectionUpdatingRef((repo) -> repo.lightweightTag(TAG_NAME, localCommit));
	...
	public void fetchReportsLockFailureWhenDestinationRefChanges()
		...
		URIish uri = newRepositoryTestConnectionUpdatingRef((repo) -> repo.update(LOCAL_BRANCH_NAME, newLocalCommit));
	...
	public void nonForcedFetchRejectedWithConcurrentLocalRefUpdate()
		...
		try (Repository repository = local.getRepository();
			...
			URIish uri = newRepositoryTestConnectionUpdatingRef((repo) -> repo.update(LOCAL_REF, updatedLocalCommit));
	...
	private <T extends AnyObjectId> URIish newRepositoryTestConnectionUpdatingRef(TestRepositoryUpdateFunc repositoryUpdateFunc) {
		TestProtocol<User> proto = registerProto((User req, Repository db) -> {
			try {
