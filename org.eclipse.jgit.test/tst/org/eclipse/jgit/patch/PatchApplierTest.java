...
public class PatchApplierTest {
	...
	public abstract static class Base extends RepositoryTestCase {
		...
		void verifyChange(ApplyPatchResult result, String name, boolean exists) throws Exception {
			if (inCore) {
				...
				RawText output = readBlob(result.appliedTree, name);
				if (!exists)
					assertNull(output);
				else
					checkInputStream(
						new ByteArrayInputStream(output.getRawContent()),
				...
				if (!exists)
					assertFalse(new File(db.getWorkTree(), name).exists());
				else
		...
		protected RawText readBlob(ObjectId treeish, String path) throws Exception {
		...
		public void testNonASCIIDel() throws Exception {
			...
			// NOSUBMIT - check non-existence of NonASCIIDel
		}
	...
	public static class InCore extends Base {
		public InCore() { super(true); }

		@Test
		public void testRenameWithHunksX() throws Exception {
			init("RenameWithHunks", true, true);
