...
public class PatchApplierTest {
	...
	public abstract static class Base extends RepositoryTestCase {
		...
		protected void verifyContent(Result result, String path, boolean exists) throws Exception {
			if (inCore) {
				byte[] output = readBlob(result.treeId, path);
				if (!exists)
					assertNull(output);
				else {
		...
		protected void checkBinary(Result result, int numberOfFiles) throws Exception {
			if (inCore) {
				assertEquals(numberOfFiles, result.paths.size());
				assertArrayEquals(postImage, readBlob(result.treeId, result.paths.get(0)));
			} else {
				assertEquals(numberOfFiles, result.files.size());
				File f = new File(db.getWorkTree(), name);
				assertEquals(f, result.files.get(0));
		...
		public void testModifyM2() throws Exception {
			...
			if (!inCore && FS.DETECTED.supportsExecute()) {
				assertEquals(1, result.files.size());
				assertTrue(FS.DETECTED.canExecute(result.files.get(0)));
			}
		...
		public void testModifyM3() throws Exception {
			...
			if (!inCore && FS.DETECTED.supportsExecute()) {
				assertFalse(
						FS.DETECTED.canExecute(result.files.get(0)));
			}
		}
		...
		public void testNonASCIIDel() throws Exception {
			...
			assertEquals("NonASCIIDel", result.paths.get(0));
		}
		...
		public void testRenameNoHunks() throws Exception {
			...
			assertTrue(result.paths.contains("RenameNoHunks"));
			assertTrue(result.paths.contains("nested/subdir/Renamed"));
		...
		public void testRenameWithHunks() throws Exception {
			...
			Result result = applyPatch();
			assertEquals(2, result.paths.size());
			assertTrue(result.paths.contains("RenameWithHunks"));
			assertTrue(result.paths.contains("nested/subdir/Renamed"));
