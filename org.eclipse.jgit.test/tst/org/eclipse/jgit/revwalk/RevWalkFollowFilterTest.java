...
public class RevWalkFollowFilterTest extends RevWalkTestCase {
	...
	public void testNoRename() throws Exception {
		...
		follow("0");
		markStart(a);
		assertEquals(a.name(), rw.next().name());
		assertNull(rw.next());
	...
	public void testSingleRename() throws Exception {
		...
		assertEquals(renameCommit.name(), rw.next().name());
		assertEquals(a.name(), rw.next().name());
		assertNull(rw.next());
	...
	public void testMultiRename() throws Exception {
		...
		assertEquals(renameCommit3.name(), rw.next().name());
		assertEquals(renameCommit2.name(), rw.next().name());
		assertEquals(renameCommit1.name(), rw.next().name());
		assertEquals(a.name(), rw.next().name());
		assertNull(rw.next());
