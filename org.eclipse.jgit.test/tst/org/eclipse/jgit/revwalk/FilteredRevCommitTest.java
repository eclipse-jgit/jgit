...
public class FilteredRevCommitTest {
	...
	@Test
	public void testCommitOverridingParents() throws Exception {
		RevCommit commit1 = tr.commit().add("foo", "foo contents\n").create();
		RevCommit commit2 = tr.commit().parent(commit1)
				.message("original message").add("bar", "bar contents")
				.create();
		...
		assertTrue(filteredCommitHead.hasOverriddenParents());
		assertFalse(commit1.hasOverriddenParents());
		assertFalse(commit2.hasOverriddenParents());
		assertFalse(commit3.hasOverriddenParents());
	}
