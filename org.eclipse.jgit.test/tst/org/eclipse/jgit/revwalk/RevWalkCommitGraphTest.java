...
public class RevWalkCommitGraphTest extends RevWalkTestCase {
	...
	@Test
	public void testChangedPathFilter_pathFilter_or_pathFilter()
			throws Exception {
	...
	@Test
	public void testChangedPathFilter_pathFilter_or_pathFilter_or_pathFilter()
			throws Exception {
		RevCommit c1 = commit(tree(file("file1", blob("1"))));
		RevCommit c2 = commit(
				tree(file("file1", blob("1")), file("file2", blob("2"))), c1);
		...
		// c2 and c3 has either modified file1 or file2 or file3, c1 did not use
		// ChangedPathFilter
		// since it has no parent
	...
	@Test
	public void testChangedPathFilter_pathFilter_or_nonPathFilter()
			throws Exception {
		RevCommit c1 = commit(tree(file("file1", blob("1"))));
		...
		// c2 modified file1, c3 defaulted positive due to ANY_DIFF, c1 did not
		// use ChangedPathFilter since it has no parent
		assertEquals(2, trf.getChangedPathFilterTruePositive());
	...
	@Test
	public void testChangedPathFilter_nonPathFilter_or_nonPathFilter()
			throws Exception {
		RevCommit c1 = commitFile("file1", "1", "master");
		RevCommit c2 = commitFile("file2", "2", "master");
	...
	@Test
	public void testChangedPathFilter_pathFilter_and_pathFilter()
			throws Exception {
		RevCommit c1 = commit(tree(file("file1", blob("1"))));
		RevCommit c2 = commit(tree(file("file2", blob("2"))), c1);
		...
		// c1 does not have a parent so didn't use changedPathFilter
		assertEquals(0, trf.getChangedPathFilterTruePositive());

		// No false positives
		assertEquals(0, trf.getChangedPathFilterFalsePositive());

		// c2 can not possibly have both file1 and file2 as treeHead at once
		assertEquals(1, trf.getChangedPathFilterNegative());
	}

	@Test
	public void testChangedPathFilter_pathFilter_and_pathFilter_and_pathFilter()
			throws Exception {
		RevCommit c1 = commit(tree(file("file1", blob("1"))));
		RevCommit c2 = commit(tree(file("file2", blob("2"))), c1);
		...
		// c1 did not use ChangedPathFilter since it has no parent.
		assertEquals(0, trf.getChangedPathFilterTruePositive());
	...
	@Test
	public void testChangedPathFilter_pathFilter_and_nonPathFilter()
			throws Exception {
		RevCommit c1 = commit(tree(file("file1", blob("1"))));
		...
		// c2 modified file1 and c1 did not counted as it does not have a parent
		assertEquals(1, trf.getChangedPathFilterTruePositive());
	...
	@Test
	public void testChangedPathFilter_nonPathFilter_and_nonPathFilter()
			throws Exception {
		RevCommit c1 = commitFile("file1", "1", "master");
