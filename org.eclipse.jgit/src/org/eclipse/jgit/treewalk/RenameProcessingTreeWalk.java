...
public class RenameProcessingTreeWalk  extends NameConflictTreeWalk {
   ...
   */
  public RenameProcessingTreeWalk(Repository repo, RenameResolver renameResolver, boolean skipRenames) {
   ...
   * @since 4.3
   */
  public RenameProcessingTreeWalk(@Nullable Repository repo, ObjectReader or, RenameResolver renameResolver, boolean skipRenames) {
    super(repo, or);
  ...
  public RenameProcessingTreeWalk(ObjectReader or,
      RenameResolver renameResolver, boolean skipRenames) {
    super(or);
    this.renameResolver = renameResolver;
    this.skipRenames = skipRenames;
  }
  ...
  public void keepTrees(Set<Integer> treesToKeep) {
    ...
    for (int i = 0; i < trees.length; i++) {
      ...
      if (!treesToKeep.contains(i) /*&& this.getTree(i, CanonicalTreeParser.class) != null*/) {
  ...
  /**
   * Returns if the entry should be processed in case of possible rename/add correlation.
   *
   ...
   * <p>Since any addition of the target path would result in {@link org.eclipse.jgit.merge.RenameResolver.RenameConflict#RENAME_ADD_CONFLICT}, only the source path renames are proccesed for possible correlations.
   * @return
   * @throws IOException
   */
  public boolean keepRenameCorrelations( )
    ...
    // We also do not want to merge them with the entiries, that are part of rename on other side.
    ...
    if (renameResolver.isBaseRename(renameEntry.targetPath)) {
      // If the target path is a rename, skip, since swapping names should be valid.
      return false;
    }
