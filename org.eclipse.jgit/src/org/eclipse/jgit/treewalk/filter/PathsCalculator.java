...
public class PathsCalculator {
	...
	/**
	 * Between two sets of paths, find paths with the narrowest shared scope.
	 * <p>
	...
	public static Optional<Set<byte[]>> OR(TreeFilter[] filters) {
		...
		return Optional.of(encodePaths(filterByLargestScopes(allPaths)));
	}
	...
	public static Optional<Set<byte[]>> AND(TreeFilter[] filters) {
		...
		for (TreeFilter f : filters) {
			...
			Set<String> simplifiedSet = filterByLargestScopes(
	...
	private static Set<String> filterByLargestScopes(Set<String> paths) {
		Set<String> result = new HashSet<>();
		for (String path : paths) {
			path = trimSlashes(path);
			String largestScope = path;
