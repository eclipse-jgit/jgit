...
public abstract class AndTreeFilter extends TreeFilter {
	...
	private static class Binary extends AndTreeFilter {
		...
		public Optional<Set<byte[]>> getPathsBestEffort() {
			java.util.List<TreeFilter> numPathFilter = Arrays
					.stream(new TreeFilter[] { a, b })
					.filter(AndTreeFilter::isSupportedPathFilter)
	...
	private static boolean isSupportedPathFilter(TreeFilter f) {
		return f instanceof PathFilter || f instanceof PathFilterGroup.Group
				|| f instanceof PathFilterGroup.Single;
	}
}
