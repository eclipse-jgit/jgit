...
public class BlameGenerator implements AutoCloseable {
	...
	private boolean processOne(Candidate n) throws IOException {
		...
		if (useCommitGraphOptimizations) {
			...
			ChangedPathTreeFilter changedPathTreeFilter = ChangedPathTreeFilter
					.create(path);
			boolean mightHaveChangedFile = changedPathTreeFilter.shouldTreeWalk(c,
					revPool);
			changedPathFilterUsed = changedPathTreeFilter.serveChangedPathFilter()
					&& c.getChangedPathFilter(revPool) != null && c.getParentCount() > 0;
