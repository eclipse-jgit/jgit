...
public class BlameGenerator implements AutoCloseable {
	...
	public boolean next() throws IOException {
		...
		for (;;) {
			...
			if (blameCache != null) {
				List<CacheRegion> fullyBlamed = blameCache.get(repository, n.sourceCommit, n.sourcePath.getPath());
				if (fullyBlamed != null) {
					RegionBlamer rb = new RegionBlamer(revPool, fullyBlamed);
					Candidate ready = rb.blame(n);
					return result(ready);
				}
			}
