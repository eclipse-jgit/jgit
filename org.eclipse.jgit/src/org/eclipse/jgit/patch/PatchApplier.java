...
public class PatchApplier {
	...
	public ApplyPatchResult applyPatch(InputStream patchInput)
		...
		try {
			...
			workTreeUpdater = inCore ?
					WorkTreeUpdater.createInCoreWorkTreeUpdater(repo, null, inserter) :
					WorkTreeUpdater.createWorkTreeUpdater(repo, null);
			...
			for (org.eclipse.jgit.patch.FileHeader fh : p.getFiles()) {
				...
				switch (type) {
					...
					case DELETE:
						f = inCore ? null : getFile(fh.getOldPath(), false);
						TreeWalk walk = getTreeWalkForFile(fh.getNewPath(), dirCache, f, fh).walk;
						walk.next();
					...
					case RENAME:
						...
						workTreeUpdater.renameFile(f, fh.getOldPath(), dest);
	...
	private FileTreeWalk getTreeWalkForFile(String path, DirCache cache, File f,
	...
	private void apply(String path, DirCache dirCache, File f, org.eclipse.jgit.patch.FileHeader fh)
			throws PatchApplyException {
		if (PatchType.BINARY.equals(fh.getPatchType())) {
			return;
		}
		try {
			FileTreeWalk fileTreeWalk = getTreeWalkForFile(path, dirCache, f, fh);
			...
			boolean convertCrLf = inCore ? false : needsCrLfConversion(f, fh);
			...
			if (PatchType.GIT_BINARY.equals(fh.getPatchType())) {
				...
				RawText raw = getRawText(f, fileStreamSupplier, fileId, path, loaded,
			...
			if (resultStreamLoaders != null) {
				...
				workTreeUpdater.insertToIndex(resultStreamLoaders.copy2,
						...
						DirCacheEntry.STAGE_0, lastModified, (int) resultStreamLoaders.copy2.getSize(),
						walk.getAttributes().get(Constants.ATTR_FILTER));
	...
	private ContentStreamLoaders applyBinary(String path, File f,
		...
		if (inCore) {
			throw new UnsupportedOperationException(
					"Applying binary patch for inCore repositories is not yet supported.");
		}
