 ...
 */
public class PatchApplier implements InCoreSupport {
  ...
  /**
   * Constructor for local repositories
   *
   * @param local repository to apply the patch in
   */
  public PatchApplier(Repository local) {
    this.inCore = false;
    if (local == null) {
  ...
  /**
   * Constructor for remote repositories
   *
   * @param remote repository to apply the patch in
   * @param tip    of the tree to apply the patch in
   * @param oi     to be used for modifying objects
   */
  public PatchApplier(Repository remote, RevTree tip, ObjectInserter oi) {
    repo = remote;
    inCore = true;
    this.tip = tip;
    inserter = oi;
  ...
  private File getFile(String path, boolean create) throws PatchApplyException {
    File f = new File(ioHandler.nonNullRepo().getWorkTree(), path);
    if (create) {
      try {
  ...
  private void apply(String path, DirCache cache, File f, org.eclipse.jgit.patch.FileHeader fh)
    ...
    try {
      ...
      if (PatchType.GIT_BINARY.equals(fh.getPatchType())) {
        ...
        safeWrite = false;
        if (!inCore) {
          ioHandler.nonNullRepo().getFS().setExecute(f, fileMode == FileMode.EXECUTABLE_FILE);
        }
      }
