...
public class Applier implements InCoreSupport {
  ...
  public ApplyPatchResult applyPatch(InputStream patchInput)
    ...
    try {
      ...
      for (org.eclipse.jgit.patch.FileHeader fh : p.getFiles()) {
        ...
        switch (type) {
          ...
          case DELETE:
            ...
            ioHandler.addDeletion(fh.getOldPath(), f.isFile(), attributes);
            break;
          case RENAME:
      ...
      patchInput.close();
      builder = ioHandler.finishBuildingAndCleanup(builder);
      result.appliedTree = ioHandler.writeCache();
    } catch (IOException e) {
      throw new PatchApplyException(
  ...
  private void apply(String path, DirCache cache, File f, org.eclipse.jgit.patch.FileHeader fh)
    ...
    try {
      ...
      if (PatchType.GIT_BINARY.equals(fh.getPatchType())) {
        ...
        RawText raw = ioHandler.getRawText(walk, loaded, fileId, path, f, fileStreamSupplier,
            convertCrLf);
      ...
      if (resultStreamLoader != null) {
        ioHandler.update(resultStreamLoader, builder, checkOutMetadata, fh.getNewPath(), f,
            safeWrite, fileMode);
      }
      if (hash != null) {
    ...
    } catch (IOException | BinaryBlobException | UnsupportedOperationException e) {
      throw new PatchApplyException(
          MessageFormat.format(JGitText.get().patchApplyException, e.getMessage()), e);
    }
  }
  ...
  private StreamLoader applyBinary(String path, File f, org.eclipse.jgit.patch.FileHeader fh,
    ...
    switch (hunk.getType()) {
      case LITERAL_DEFLATED:
        ...
        return ioHandler.getStreamLoader(() -> inflated, hunk.getSize());
      case DELTA_DEFLATED:
        ...
        return ioHandler.getStreamLoader(() -> hashed, finalSize);
      default:
        throw new UnsupportedOperationException(
  ...
  private StreamLoader applyText(RawText rt, org.eclipse.jgit.patch.FileHeader fh)
    ...
    try (OutputStream out = buffer) {
      ...
      return ioHandler.getStreamLoader(buffer::openInputStream, buffer.length());
    }
  }
