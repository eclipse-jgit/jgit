...
public class Applier implements InCoreSupport {
  ...
  public Applier(Repository local, boolean shouldUpdateIndex) {
    this.inCore = false;
    if (local == null) {
  ...
  public Applier(Repository remote, ObjectInserter oi) {
    repo = remote;
    inCore = true;
    inserter = oi;
    reader = oi.newReader();
   ...
   */
  public void applyPatch(InputStream patchInput) throws PatchFormatException,
      PatchApplyException {
    try {
      try {
        inCoreHandler = InCoreSupport.initInCoreHandler(repo, inserter, reader, inCore,
            shouldUpdateIndex, attributes);
        ...
        for (FileHeader fh : p.getFiles()) {
          ...
          switch (type) {
            ...
            case DELETE:
              ...
              inCoreHandler.addDeletion(fh.getOldPath(), f.isFile(), attributes);
              break;
            case RENAME:
              f = inCore ? null : getFile(fh.getOldPath(), false);
  ...
  private void apply(String path, DirCache cache,
      File f, FileHeader fh) throws PatchApplyException {
    if (PatchType.BINARY.equals(fh.getPatchType())) {
      return;
    }
    // DO NOT SUBMIT - for inCore - repo is null. I should have a snippet for the right walk.
    try (TreeWalk walk = new TreeWalk(repo)) {
      ...
      if (walk.next()) {
        ...
        if (inCore) {
          fileId = fh.getOldId().toObjectId();
          ObjectLoader loader = LfsFactory.getInstance().applySmudgeFilter(
              repo, reader.open(fileId, OBJ_BLOB),
              null);
      ...
      if (PatchType.GIT_BINARY.equals(fh.getPatchType())) {
        resultStreamLoader = applyBinary(repo, path, f, fh,
            fileStreamSupplier, fileId,
            checkOutMetadata);
        safeWrite = true;
      } else {
        RawText raw = inCoreHandler.getRawText(walk, loaded, fileId, path, f, fileStreamSupplier,
            convertCrLf);
        resultStreamLoader = applyText(path, raw, fh, checkOutMetadata);
        safeWrite = false;
        if (!inCore) {
          repo.getFS().setExecute(f, fileMode == FileMode.EXECUTABLE_FILE);
        }
      }
      inCoreHandler.update(resultStreamLoader, builder, checkOutMetadata, fh.getNewPath(), f,
          safeWrite, fileMode);
  ...
  private void checkOid(ObjectId baseId, ObjectId id, ChangeType type, File f,
    ...
    if (id != null) {
      ...
      // DO NOT SUBMIT - is this code branch for file (content) deletion?
  ...
  private StreamLoader applyBinary(Repository repository, String path,
      File f, FileHeader fh, InCoreHandler.StreamSupplier inputSupplier, ObjectId id,
      ...
      switch (hunk.getType()) {
        case LITERAL_DEFLATED:
          ...
          try (InputStream inflated = new SHA1InputStream(hash,
            ...
            return inCoreHandler.getStreamLoader(() -> inflated, hunk.getSize());
          }
        case DELTA_DEFLATED:
          ...
          try (BinaryDeltaInputStream input = new BinaryDeltaInputStream(
            ...
            try (SHA1InputStream hashed = new SHA1InputStream(hash,
              ...
              return inCoreHandler.getStreamLoader(() -> hashed, finalSize);
            }
          }
        default:
          break;
    ...
    // DO NOT SUBMIT - might need to throw an error.
    return null;
  }

  private StreamLoader applyText(String path, RawText rt, FileHeader fh, CheckoutMetadata
      checkOutMetadata)
    ...
    if (oldLines.equals(newLines)) {
      // DO NOT SUBMIT - wrong output.
      return null; // Unchanged; don't touch the file.
    }
    ...
    try (OutputStream out = buffer) {
      ...
      return inCoreHandler.getStreamLoader(buffer::openInputStream, buffer.length());
    }
    // DO NOT SUBMIT - handle the commented out code.
