...
public class Applier implements InCoreSupport {
  ...
  public ApplyPatchResult applyPatch(InputStream patchInput)
    ...
    try {
      ...
      inCoreHandler = InCoreSupport.initInCoreHandler(repo, inserter, reader, inCore, attributes);
      DirCache cache = inCoreHandler.getLockedDirCache();
      ...
      patchInput.close();
      builder = inCoreHandler.finishBuildingAndCleanup(builder);
      result.appliedTree = inCoreHandler.writeCache();
    } catch (IOException e) {
      throw new PatchApplyException(
  ...
  private StreamLoader applyBinary(String path, File f, org.eclipse.jgit.patch.FileHeader fh,
      InCoreHandler.StreamSupplier inputSupplier, ObjectId id, SHA1 hash)
      throws PatchApplyException, IOException {
