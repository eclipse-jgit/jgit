...
class GpgBinary {
	 ...
	 Path getPath() throws IOException {
		...
		try {
			...
			throw new IOException(MessageFormat.format(
					JGitText.get().externalGpgInvalidPath, program), e);
		}
		...
		if (exe.getNameCount() == 1) {
			...
			throw new IOException(
					JGitText.get().externalGpgSignerGpgNotFound);
		}
		...
		throw new IOException(JGitText.get().externalGpgSignerGpgNotFound);
	}
	...
	private static String findProgramViaLoginShell(String program, String path,
		...
		try {
			...
			LOG.warn(JGitText.get().externalGpgSignerCannotSearch, e);
			return null;
		}
	}
	...
	private static void logSkipNotAccessiblePath(String path,
		...
		try {
			LOG.warn(MessageFormat.format(
					JGitText.get().externalGpgSignerSkipNotAccessiblePath,
					path), e);
