...
public abstract class PackedRefsWriter {
	 ...
	 * @param refDb
	 *            refDb where the packed-refs file should be written.
	 *
	 * @throws IOException
	...
	public PackedRefsWriter(Collection<Ref> refs,
			EnumSet<PackedRefsTrait> traits, RefDatabase refDb)
			throws IOException {
		this(refs, traits);
		applyMissingTraits(refDb);
	}
	...
	private void applyMissingTraits(RefDatabase refDb) throws IOException {
		if (!traits.contains(PackedRefsTrait.SORTED)) {
			refs = RefComparator.sort(refs);
