...
public abstract class RefWriter {
	 ...
	 * @param traits
	 *            traits to be included on the packed-refs file.
	 *
	 * @since 7.7
	 */
	...
	public RefWriter(Collection<Ref> refs) {
		this.refs = RefComparator.sort(refs);
		traits = EnumSet.of(PackedRefsTrait.SORTED);
		if (arePeeled(refs)) {
	...
	public RefWriter(Map<String, Ref> refs) {
		this.refs = refs instanceof RefMap ? refs.values() : RefComparator.sort(refs.values());
		traits = EnumSet.of(PackedRefsTrait.SORTED);
		if (arePeeled(this.refs)) {
			traits.add(PackedRefsTrait.PEELED);
		}
	}
	...
	public RefWriter(RefList<Ref> refs) {
		this.refs = refs.asList();
		this.traits = EnumSet.of(PackedRefsTrait.SORTED);
		if (arePeeled(this.refs)) {
			traits.add(PackedRefsTrait.PEELED);
		}
	}
	...
	@Deprecated(since = "7.7")
	private boolean arePeeled(Collection<Ref> refs) {
		for (Ref r : refs) {
