...
public class TagBuilder extends ObjectBuilder {
		...
		try (OutputStreamWriter w = new OutputStreamWriter(os,
			...
			if (getGpgSignature() != null) {
				if (msg != null && !msg.isEmpty() && !msg.endsWith("\n")) { //$NON-NLS-1$
					// If signed, the message *must* end with an LF, otherwise
					// signature verification will fail. (The signature will
					// have been computed over the payload containing the
					// message without LF, but will be verified against a
					// payload with the LF.) The signature must start on a new
					// line.
	...
	public String toString() {
		...
		if (getEncoding() != null
				&& !References.isSameObject(getEncoding(), UTF_8)) {
			r.append("encoding ");
			r.append(getEncoding().name());
			r.append("\n");
		}
		...
		r.append(
				getGpgSignature() != null ? getGpgSignature().toExternalString()
						: "");
		r.append("}");
		return r.toString();
	}
