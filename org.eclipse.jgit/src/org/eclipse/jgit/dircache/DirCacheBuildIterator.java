 ...
 * TreeWalk. Unless {@link #copyOnSkip} is off, any entry which appears in the
 * source DirCache and which is skipped by the TreeFilter is automatically
 * copied into {@link org.eclipse.jgit.dircache.DirCacheBuilder}, thus retaining
 * it in the newly updated index.
 * <p>
...
public class DirCacheBuildIterator extends DirCacheIterator {
	 ...
	 *
	 * <p>
	 * When walked by {@link org.eclipse.jgit.treewalk.TreeWalk}, the created
	 * iterator will copy entries, matched by
	 * {@link org.eclipse.jgit.treewalk.filter.TreeFilter} to the
	 * {@link org.eclipse.jgit.dircache.DirCacheBuilder}
	 *
	 * @param dcb
	 ...
	 * Create a new iterator for an already loaded DirCache instance.
	 * <p>
	 * With {@code copyOnSkip} false, this constructor can be used to create an
	 * iterator on {@link DirCacheBuilder} that was walked before and thus
	 * contains all interesting entries already.
	 *
	 * @param dcb
	 *            the cache builder for the cache to walk. The cache must be
	 *            already loaded into memory.
	 * @param copyOnSkip
	 *            whether the iterator should copy the matched entries to
	 *            {@link #builder}
	 */
	...
	DirCacheBuildIterator(final DirCacheBuildIterator p, final DirCacheTree dct,
			boolean copyOnSkip) {
		super(p, dct);
		builder = p.builder;
		this.copyOnSkip = copyOnSkip;
