 ...
 * check determines that the input is not binary but text with CR/LF, the stream
 * behaves like an {@link AutoCRLFOutputStream}.
 * </p>
 *
 * @since 4.3
 */
public class AutoLFOutputStream extends OutputStream {
	...
	private void decideMode() throws IOException {
		if (detectBinary) {
			...
			if (!isBinary) {
				isCrLf = RawText.isCrLfText(binbuf, binbufcnt);
			}
			detectBinary = false;
		}
