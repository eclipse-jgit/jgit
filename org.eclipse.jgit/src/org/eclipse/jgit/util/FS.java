		...
		private static Duration getFsResolution(FileStore s, Path dir,
			...
			while (high > low) {
				...
				if (cmp > 0) {
					...
					if (diff.isZero() || diff.isNegative()) {
						LOG.warn(
								"{}: inconsistent duration from file timestamps on {}, {}: {} > {}, but diff = {}. " //$NON-NLS-1$
										+ "Aborting measurement at resolution {}", //$NON-NLS-1$
								Thread.currentThread(), s, dir, t2, t1, diff,
								last);
						break;
					} else if (diff.compareTo(last) > 0) {
						LOG.warn(
								"{}: inconsistent duration from file timestamps on {}, {}: " //$NON-NLS-1$
										+ "diff = {} > {} (last good value). Aborting measurement.", //$NON-NLS-1$
					...
					LOG.warn(
							"{}: got smaller file timestamp on {}, {}: {} < {}. " //$NON-NLS-1$
									+ "Aborting measurement at resolution {}", //$NON-NLS-1$
