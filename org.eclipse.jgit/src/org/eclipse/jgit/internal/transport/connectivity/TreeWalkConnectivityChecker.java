 ...
 *
 * Instead of marking objects interesting and uninteresting and parsing new
 * subtrees and blobs to propagate those states, for each commit and its direct
 * parents, this algorithm parses tree objects at common paths and compares the
 * items in those trees.
 *
 * Standard graph coloring is more efficient than this algorithm when the
 * parents of new commits are all in the advertised "haves", touching O(objects
 * in the new pack), and less efficient otherwise, where it touches either
 * O(tree objects in checkout) or O(all objects in checkout). This algorithm
 * delegates to the standard algorithm when all parent commits are in advertised
 * haves.
 *
 ...
 * for the client, move on to verifying tree connectivity. Otherwise, perform a
 * reachability check to make sure the client has access to the unadvertised
 * parent commit, and proceed to verifying tree connectivity if that succeeds.
 *
...
public class TreeWalkConnectivityChecker implements ConnectivityChecker {
	 ...
	 */
	public TreeWalkConnectivityChecker() {
	}
	 ...
	 *            set to populate with parent commits outside the pack
	 * @param pm
	 *            progress monitor
	 * @return list of new commits to verify
	 * @throws IOException
	...
	private void checkThinPackBases(RevWalk rw,
		...
		try (ObjectWalk ow = new ObjectWalk(rw.getObjectReader())) {
			...
			List<RevObject> targetObjs = StreamSupport
					...
					.map(id -> parseAnyUnchecked(rw, id))
			...
			try (TreeWalk tw = new TreeWalk(rw.getObjectReader())) {
				...
				while (tw.next()) {
					...
					if (!tw.isSubtree()) {
						...
						// database, but the map lookup is faster than the
						// database search, so try the map lookup first.
