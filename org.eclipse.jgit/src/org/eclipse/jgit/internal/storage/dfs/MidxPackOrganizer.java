 ...
 * </ul>
 */
public class MidxPackOrganizer {

	private MidxPackOrganizer() {
	}
	...
	public static List<DfsPackDescription> useMidx(
		...
		Set<DfsPackDescription> badMidxs = new HashSet<>();
		for (DfsPackDescription midx : midxs) {
			...
			if (!inputPacks.containsAll(coveredPacks)) {
				// This midx references packs not in the pack db. Skip it.
				badMidxs.add(midx);
			}
		...
		return packs.stream().filter(
				d -> !allCoveredPacks.contains(d) && !badMidxs.contains(d))
				.collect(Collectors.toList());
	}
