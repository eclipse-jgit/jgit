...
public final class MidxPackList {
	 ...
	 */
	PackSet<DfsPackFileMidx> findAllImpactedMidxs(DfsPackFile pack) {
	 ...
	 */
	public PackSet<DfsPackFileMidx> findAllImpactedMidxs(
			List<DfsPackFile> toRemove) {
		if (toRemove.isEmpty()) {
			return PackSet.EMPTY;
		}
		...
		PackSet<DfsPackFileMidx> impactedMidxs = new PackSet(List.of());
	...
	private static boolean containsAny(List<DfsPackFile> inMidx,
			List<DfsPackFile> toRemove) {
		Set<DfsPackFile> inMidxSet = new PackSet(inMidx);
