...
public final class DfsPackFile extends BlockBasedFile {
	...
	int getObjectSizeIndexThreshold(DfsReader ctx) throws IOException {
		PackObjectSizeIndex objectSizeIndex = getObjectSizeIndex(ctx);
		if (objectSizeIndex == null) {
	...
	private DfsBlockCache.Ref<PackObjectSizeIndex> loadObjectSizeIndex(
		...
		ReadableChannel rc;
		try {
			rc = ctx.db.openFile(desc, OBJECT_SIZE_INDEX);
		} catch (IOException e) {
			// No object size index in this pack
			return new DfsBlockCache.Ref<>(objectSizeIndexKey, REF_POSITION, 0, null);
		}
