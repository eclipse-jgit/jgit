...
public final class DfsPackFile extends BlockBasedFile {
	...
	private PackIndex idx(DfsReader ctx) throws IOException {
		...
		try {
			index = indexFactory.getIndexes().index(ctx);
			if (index == null) {
	...
	public PackReverseIndex getReverseIdx(DfsReader ctx) throws IOException {
		...
		reverseIndex = indexFactory.getIndexes().reverseIndex(ctx);
		if (reverseIndex == null) {
			throw new IOException(
	 ...
	 * DfsBlockCache
	 */
	public final static class CachedStreamIndexFactory implements IndexFactory {
		private final CachedStreamIndexes indexes;
		...
		public CachedStreamIndexFactory(DfsBlockCache cache,
			...
			this.indexes = new CachedStreamIndexes(cache, desc);
		}
	 ...
	 * in DfsBlockCache.
	 *
	 * @implSpec Subclasses can access the cache instance, mainly to check if a
	 *           value is already there before triggering the load. In this
	 *           implementation, {@link #reverseIndex(DfsReader)} calls
	 *           {@link #index(DfsReader)}.
	 */
	public static class CachedStreamIndexes implements IndexFactory.Indexes {

		/** Instance of Dfs cache to keep the loaded references */
		protected final DfsBlockCache cache;
		 ...
		 * @param desc This factory loads indexes for this package
		 */
		public CachedStreamIndexes(DfsBlockCache cache,
				DfsPackDescription desc) {
			this.cache = cache;
			this.desc = desc;
		}
