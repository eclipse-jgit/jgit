...
public final class DfsPackFile extends BlockBasedFile {
	...
	private static final BitmapStorageLoader DEFAULT_BITMAP_LOADER = new StreamPackBitmapIndexLoader();
	...
	private final BitmapStorageLoader bitmapLoader;
	...
	DfsPackFile(DfsBlockCache cache, DfsPackDescription desc, BitmapStorageLoader bitmapLoader) {
	...
	public PackBitmapIndex getBitmapIndex(DfsReader ctx) throws IOException {
		...
		if (!bitmapLoader.keepInDfs(desc)) {
			BitmapStorageLoader.BitmapLoadResult result = bitmapLoader
					.loadBitmaps(ctx, this);
	...
	interface BitmapStorageLoader {
		boolean hasBitmaps(DfsPackDescription desc);
		boolean keepInDfs(DfsPackDescription desc);
		BitmapLoadResult loadBitmaps(DfsReader ctx, DfsPackFile pack) throws IOException;

		class BitmapLoadResult {
			PackBitmapIndex bitmapIndex;
			long bytesRead;

			private BitmapLoadResult(PackBitmapIndex packBitmapIndex, long bytesRead) {
	...
	static class StreamPackBitmapIndexLoader implements BitmapStorageLoader {
		...
		@Override
		public BitmapLoadResult loadBitmaps(DfsReader ctx, DfsPackFile pack) throws IOException {
			...
			try (ReadableChannel rc = ctx.db.openFile(desc, BITMAP_INDEX)) {
				...
				return new BitmapLoadResult(bmidx, size);
			} catch (EOFException e) {
				throw new IOException(MessageFormat.format(
						DfsText.get().shortReadOfIndex,
