...
public final class DfsPackFile extends BlockBasedFile {
	...
	private IndexRef<PackIndex> loadPackIndex(DfsReader ctx)
			throws IOException {
		try {
			ctx.stats.readIdx++;
			...
			try (ReadableChannel rc = ctx.db.openFile(desc, INDEX)) {
				...
				return new IndexRef<>(idx, idx.getObjectCount() * REC_SIZE);
			} finally {
	...
	private static final class IndexRef<V> {
		final V ref;
		final long size;
		IndexRef(V ref, long size) {
			this.ref = ref;
			this.size = size;
		}
	}
