...
public final class DfsPackFile extends BlockBasedFile {
	...
	/**
	 * Return minimum size for an object to be included in the object size index
	 *
	 * @param ctx
	 *            reader context to support reading from the backing store if
	 *            the object size index is not already loaded in memory.
	 * @return minimum size for indexing in bytes, -1 if no object size index in
	 *         this pack
	 */
	int getObjectSizeIndexThreshold(DfsReader ctx) {
		PackObjectSizeIndex objectSizeIndex;
		try {
			objectSizeIndex = getObjectSizeIndex(ctx);
		} catch (IOException e) {
			return -1;
		}
	...
	private DfsBlockCache.Ref<PackObjectSizeIndex> loadObjectSizeIndex(
		...
		try (ReadableChannel rc = ctx.db.openFile(desc, OBJECT_SIZE_INDEX)) {
			...
			return new DfsBlockCache.Ref<>(objectSizeIndexKey, REF_POSITION,
