...
public class DfsBlockCacheConfig {
	...
	private void setPackExtConfigs(Config config) {
		...
		for (String subSection : subSections) {
			...
			Set<PackExt> packExtsIntersection = intersection(extensionsSeen,
					cacheConfig.packExts);
			if (packExtsIntersection.size() > 0) {
				String intersectingPackExts = packExtsIntersection.stream()
						.map(PackExt::toString)
						.collect(Collectors.joining(","));
				throw new IllegalArgumentException(MessageFormat.format(
						JGitText.get().duplicatePackExtensionsSet,
						CONFIG_CORE_SECTION, subSection,
						CONFIG_KEY_PACK_EXTENSIONS, intersectingPackExts));
			}
