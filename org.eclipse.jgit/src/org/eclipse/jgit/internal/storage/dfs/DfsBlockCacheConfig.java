...
public class DfsBlockCacheConfig {
	...
	static class DfsBlockCachePackExtConfig {
		...
		private static List<DfsBlockCachePackExtConfig> fromConfigs(
				Config config) {
			Set<String> extensionsSeen = new HashSet<>();
			...
			for (String subSection : subSections) {
				...
				for (String extension : extensions) {
					if (extensionsSeen.contains(extension)) {
						throw new IllegalArgumentException(MessageFormat.format(
								JGitText.get().duplicatePackExtensionsSet,
								CONFIG_CORE_SECTION, subSection,
								CONFIG_KEY_PACK_EXTENSIONS, extension));
					}
					extensionsSeen.add(extension);
					try {
				...
				dfsBlockCacheConfig.fromConfig(CONFIG_CORE_SECTION, subSection,
						config);
				cacheConfigs.add(new DfsBlockCachePackExtConfig(packExts,
						dfsBlockCacheConfig));
			}
			return cacheConfigs;
		}
