...
public class WindowCache {
	...
	/** return true if more processing should be done */
	private boolean removeNextBlock(int numRemoved) {
		...
		/* Ensure threads never work on more than a full set of blocks (the equivalent of removing one Pack) */
	...
	private void removeBlock(Set<Pack> packs, int block) {
		for (int s = block * removalBlockSize; s < (block + 1) * removalBlockSize && s < tableSize; s++) {
