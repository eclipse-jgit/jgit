...
public class MidxWriter {
	 ...
	 */
	public static void writeMidx(ProgressMonitor pm, Repository db,
	...
	private static void createAndAttachBitmaps(ProgressMonitor pm,
			Repository db, File midxBitmapsOut, byte[] checksum,
		...
		RefAdvancerWalk adv = new RefAdvancerWalk(db, c -> byId.contains(c));
		...
		calculator.calculate(db.newObjectReader(), NullProgressMonitor.INSTANCE,
				commitCount, inPack, new HashSet<>(), writeBitmaps);
		try (FileOutputStream out = new FileOutputStream(
