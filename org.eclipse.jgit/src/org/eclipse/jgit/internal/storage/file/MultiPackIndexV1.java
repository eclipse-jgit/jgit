...
public class MultiPackIndexV1 implements MultiPackIndex {
	...
	public String toString() {
		return "MultiPackIndexV1 {" //$NON-NLS-1$
				+ "idx=" //$NON-NLS-1$
				+ idx
				+ ", packfileNames=" //$NON-NLS-1$
				+ Arrays.toString(packfileNames)
				+ ", bitmappedPackfiles=" //$NON-NLS-1$
				+ byteArrayToString(bitmappedPackfiles)
				+ ", objectOffsets=" //$NON-NLS-1$
				+ objectOffsets + '}';
	}
	...
	public boolean containsPack(Pack pack) {
		if (packNames == null) {
			packNames = new HashSet<>();
			for (String packFileName : getPackFileNames()) {
				packNames.add(new PackFile(packDir.getDirectory(),
						packFileName).getId());
			}
		}
	...
	private Optional<Pack> getValidPack(String packFileName) {
		PackFile packFile = new PackFile(packDir.getDirectory(), packFileName);
		Pack pack = packDir.getPacksById().get(packFile.getId());
		if (pack != null) {
			LOG.info("Using pack {} found in MIDX", pack); //$NON-NLS-1$
			return Optional.of(pack);
		}
		LOG.warn(
				"Ignoring packfile {} registered in MIDX and not found in scanned packs", //$NON-NLS-1$
				packFileName);
		return Optional.empty();
	}
}
