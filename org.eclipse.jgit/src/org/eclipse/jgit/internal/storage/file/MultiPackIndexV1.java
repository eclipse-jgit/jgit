...
public class MultiPackIndexV1 implements MultiPackIndex {
	...
	Optional<PackAndOffset> findOffset(AnyObjectId objectId) {
		ObjectOffsets.ObjectOffset objectOffset = getObjectOffset(objectId);
		String packFileName = getPackFileName(objectOffset);
		Optional<Pack> optionalPack = getValidPack(packFileName);
		return optionalPack.map(
				pack -> new PackAndOffset(pack, objectOffset.getOffset()));
	}
	 ...
	 */
	ObjectOffsets.ObjectOffset getObjectOffset(AnyObjectId objectId) {
