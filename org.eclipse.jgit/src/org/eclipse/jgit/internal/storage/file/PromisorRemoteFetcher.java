...
class PromisorRemoteFetcher implements PromisorObjectFetcher {
	...
	PromisorRemoteFetcher(FileRepository repo) {
		this.repo = repo;
	}
	...
	public boolean fetch(Collection<ObjectId> ids) throws IOException {
		...
		try (Transport tn = Transport.open(repo, remote)) {
			// Let callers supply credentials, a timeout, proxy or other
			// transport-specific settings for the on-demand fetch. The
			// transport already defaults to CredentialsProvider.getDefault();
			// this hook covers per-repository configuration (e.g. the
			// credentials a CloneCommand was given). Applied before the filter
			// so the persisted partial-clone filter always wins.
			TransportConfigCallback config = repo.getPromisorTransportConfig();
			if (config != null) {
				config.configure(tn);
			}
