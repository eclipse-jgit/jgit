...
public class PackBitmapIndexBuilder extends BasePackBitmapIndex {
	...
	/**
	 * Process a commit and prepares its bitmap to write to bitmap index file.
	 *
	 * @param c
	...
	public void processBitmapForWrite(BitmapCommit c, Bitmap bitmap,
		...
		if (c.isAddToIndex()) {
			// To reduce memory usage, only keeps the bitmap when the commit
			// requires to do. And the commit only requires to do so when it's
			// old enough, i.e., far enough from branch tip.
