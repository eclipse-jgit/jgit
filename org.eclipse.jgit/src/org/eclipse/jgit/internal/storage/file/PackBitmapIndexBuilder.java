...
public class PackBitmapIndexBuilder extends BasePackBitmapIndex {
	...
	/**
	 * Get a stream of the inflated (NON-xor-compressed) bitmaps. Using this
	 * function allows incremental inflation of the xor-compressed bitmaps
	 * without having to expand more than {@link #MAX_XOR_OFFSET_SEARCH} bitmaps
	 * in memory at one time.
	 *
	 * @param xorCompressedEntries
	 *            list of xor-compressed entries to inflate
	 *
	 * @return a stream the inflated (NON-xor-compressed) bitmaps that will be
	 *         written
	 */
	public static Stream<StoredEntry> getEwahCompressedBitmapStream(
			List<StoredEntry> xorCompressedEntries) {
		return getEwahCompressedBitmapStream(xorCompressedEntries,
	 ...
	 * Get a stream of the inflated (NON-xor-compressed) bitmaps. Using this
	 * function allows incremental inflation of the xor-compressed bitmaps
	 * without having to expand more than {@code maxXorOffset} bitmaps in memory
	 * at one time.
	 *
	 * @param xorCompressedEntries
	 *            list of xor-compressed entries to inflate
	 * @param maxXorOffset
	 *            maximum offset from a bitmap to the bitmap it is
	 *            xor-compressed against
	 *
	 * @return a stream the inflated (NON-xor-compressed) bitmaps that will be
	 *         written
	 */
	static Stream<StoredEntry> getEwahCompressedBitmapStream(
			List<StoredEntry> xorCompressedEntries, int maxXorOffset) {
		LinkedList<StoredEntry> inflatedEntriesBuffer = new LinkedList<>();
		return xorCompressedEntries.stream().map(bitmapEntry -> {
			while (inflatedEntriesBuffer.size() > maxXorOffset) {
