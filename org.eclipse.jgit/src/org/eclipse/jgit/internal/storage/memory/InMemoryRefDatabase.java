...
public class InMemoryRefDatabase extends RefDatabase implements RefCache {
	...
	/**
	 * Create an InMemoryRefDatabase wrapping another RefDatabase and serving as
	 * a write-through cache for the wrapped RefDatabase.
	 *
	 * @param refDir
	 *            the RefDirectory this InMemoryRefDatabase wraps as a
	 *            write-through cache
	 */
	...
	public Map<String, Ref> getRefs(String prefix) throws IOException {
		...
		if (isValidPrefix(prefix)) {
			for (Entry<String, Ref> e : cache.getWithPrefix(prefix)
					.entrySet()) {
				if (e.getKey().endsWith(Constants.LOCK_SUFFIX)) {
					continue;
				}
	...
	private static boolean isValidPrefix(String prefix) {
		return prefix == RefDatabase.ALL || (prefix.length() > 0
