 ...
 * {@link #setExpireMinUpdateIndex(long)} and {@link #setExpireMaxUpdateIndex(long)} are
 ...
 * {@link #setExpireOldestReflogTimeMillis(long)}, or disable the log altogether with
...
public class ReftableCompactor {
	...
	private long expireMinUpdateIndex = 0;
	private long expireMaxUpdateIndex = Long.MAX_VALUE;
	private long expireOldestReflogTimeMillis;
	private Stats stats;
	 ...
	 * @return {@code this}
	 */
	public ReftableCompactor setExpireMinUpdateIndex(long min) {
		expireMinUpdateIndex = min;
		return this;
	}
	 ...
	 * @return {@code this}
	 */
	public ReftableCompactor setExpireMaxUpdateIndex(long max) {
		expireMaxUpdateIndex = max;
		return this;
	}
	 ...
	 * @return {@code this}
	 */
	public ReftableCompactor setExpireOldestReflogTimeMillis(long timeMillis) {
		expireOldestReflogTimeMillis = timeMillis;
		return this;
	}
	...
	private void mergeLogs(MergedReftable mr) throws IOException {
		if (expireOldestReflogTimeMillis == Long.MAX_VALUE) {
			return;
		}
		...
		try (LogCursor lc = mr.allLogs()) {
			while (lc.next()) {
				...
				if (updateIndex > expireMaxUpdateIndex || updateIndex < expireMinUpdateIndex) {
					continue;
				}
				...
				if (who.getWhen().getTime() >= expireOldestReflogTimeMillis) {
					writer.writeLog(
							refName,
							updateIndex,
							who,
