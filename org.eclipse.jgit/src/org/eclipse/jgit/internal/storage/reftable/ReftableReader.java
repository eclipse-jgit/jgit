...
public class ReftableReader extends Reftable implements AutoCloseable {
	...
	private class RefCursorImpl extends RefCursor {
		...
		private String latestSkippedPrefix;
		private Ref ref;
		BlockReader block;
		...
		public boolean next() throws IOException {
			// when skipping past a prefix, we sometimes may not skip fully. This is needed to ensure that
			// the returned ref is not part of the skipped prefix.
			while (nextWrapper()) {
	...
	private class ObjCursorImpl extends RefCursor {
		...
		@Override
		/** The implementation here would not be efficient complexity-wise since it expected that there
		 * are a small number of refs that match the same object id. In such case it's better to not
		 * even use this method (as the caller might expect it to be efficient). */
		public void seekPastPrefix(String prefixName) throws IOException {
