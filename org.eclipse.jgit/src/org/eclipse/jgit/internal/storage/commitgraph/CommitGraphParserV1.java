...
class CommitGraphParserV1 implements CommitGraph {
	...
	CommitGraphParserV1(InputStream fd, byte[] hdr) throws IOException {
		...
		commitDataLength = hashLength + COMMIT_DATA_WIDTH;
		...
		long maxChunkOffset = chunks.get(numberOfChunks).offset;
		for (int i = 0; i < numberOfChunks; i++) {
			int chunkId = chunks.get(i).id;
			long chunkOffset = chunks.get(i).offset;
			long length = chunks.get(i + 1).offset - chunkOffset;
			long lengthRead;
			if (chunkOffset < 0 || chunkOffset > maxChunkOffset) {
				throw new CommitGraphFormatException(MessageFormat.format(
						JGitText.get().commitGraphChunkImproperOffset,
						Integer.toHexString(chunkId),
						Long.valueOf(chunkOffset)));
			}
			switch (chunkId) {
			case CHUNK_ID_OID_FANOUT:
				lengthRead = loadChunkOidFanout(fd);
				break;
			case CHUNK_ID_OID_LOOKUP:
				lengthRead = loadChunkDataBasedOnFanout(fd, hashLength,
						oidLookup);
				break;
			case CHUNK_ID_COMMIT_DATA:
				lengthRead = loadChunkDataBasedOnFanout(fd, commitDataLength,
						commitData);
				break;
			case CHUNK_ID_EXTRA_EDGE_LIST:
				lengthRead = loadChunkExtraEdgeList(fd, length);
				break;
			default:
				throw new CommitGraphFormatException(MessageFormat.format(
			...
			if (length != lengthRead) {
				throw new CommitGraphFormatException(MessageFormat.format(
						JGitText.get().commitGraphChunkImproperOffset,
						Integer.toHexString(chunks.get(i + 1).id),
						Long.valueOf(chunks.get(i + 1).offset)));
			}
		}
	...
	public CommitGraph.CommitData getCommitData(int graphPos) {
		...
		int[] parents = pList;
		return new CommitData(tree, parents, commitTime, generation);
	}
