 ...
 * <li>Get the generation number of commit.</li>
 * </ul>
 */
public class GraphCommits implements Iterable<RevCommit> {
	...
	public static GraphCommits fromWalk(ProgressMonitor pm,
		...
		pm.endTask();
		return new GraphCommits(pm, commits);
	}
	 ...
	 */
	private GraphCommits(ProgressMonitor pm, List<RevCommit> commits) {
	...
	private void computeGenerationNumbers(ProgressMonitor pm) {
		pm.beginTask(JGitText.get().computingCommitGeneration, size());
