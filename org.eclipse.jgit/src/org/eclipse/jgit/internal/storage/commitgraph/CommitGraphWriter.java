...
public class CommitGraphWriter {
	 ...
	 *
	 * @param preparingMonitor
	 *            progress monitor to report the progress of preparing.
	 * @param wants
	 *            the list of wanted objects, writer walks commits starting at
	 *            these. Must not be {@code null}.
	 * @throws IOException
	 */
	public void prepareCommitGraph(ProgressMonitor preparingMonitor,
			@NonNull Set<? extends ObjectId> wants) throws IOException {
		BlockList<RevCommit> commits = findCommits(preparingMonitor, wants);
		if (computeGeneration) {
			computeGenerationNumbers(preparingMonitor, commits);
		}
	}
	...
	private BlockList<RevCommit> findCommits(ProgressMonitor findingMonitor,
		...
		RevCommit c;
		findingMonitor.beginTask(JGitText.get().findingCommitsForCommitGraph,
	...
	private void computeGenerationNumbers(
		...
		computeGenerationMonitor.beginTask(
				JGitText.get().computingCommitGeneration, commits.size());
		...
		computeGenerationMonitor.endTask();
	}
