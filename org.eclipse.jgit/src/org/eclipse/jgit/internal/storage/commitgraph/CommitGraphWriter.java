...
public class CommitGraphWriter {
	...
	private void prepareCommitGraph(@NonNull Set<? extends ObjectId> wants)
			throws IOException {
		BlockList<RevCommit> commits = findCommits(wants);
		computeGenerationNumbers(commits);
	}
	...
	private void writeTo(OutputStream commitGraphStream) throws IOException {
		...
		pm.beginTask(MessageFormat.format(JGitText.get().writingOutCommitGraph,
				Integer.valueOf(chunks.length)), (int) writeCount);
	...
	private BlockList<RevCommit> findCommits(Set<? extends ObjectId> wants)
			throws IOException {
	...
	private void computeGenerationNumbers(List<RevCommit> commits)
			throws MissingObjectException {
		pm.beginTask(JGitText.get().computingCommitGeneration, commits.size());
