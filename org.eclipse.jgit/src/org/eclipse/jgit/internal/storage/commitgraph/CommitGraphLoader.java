...
public class CommitGraphLoader {
	 ...
	 */
	public static CommitGraph open(File graphFile, boolean readBloomFilter) throws FileNotFoundException,
			CommitGraphFormatException, IOException {
		try (SilentFileInputStream fd = new SilentFileInputStream(graphFile)) {
			try {
				return read(fd, readBloomFilter);
	 ...
	 */
	public static CommitGraph read(InputStream fd, boolean readBloomFilterChunks)
