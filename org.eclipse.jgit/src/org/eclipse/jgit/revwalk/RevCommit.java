...
public class RevCommit extends RevObject {
	...
	final void parseInGraph(RevWalk walk) {
		...
		if (data == null) {
			// parseInGraph was called because we know this commit's
			// position in the commit graph. If now the commit-graph doesn't
			// know about it, something went wrong.
	...
	final int getEffectiveGeneration() {
		...
		if (gen == Constants.COMMIT_GENERATION_NOT_COMPUTED) {
