...
public class RevWalk implements Iterable<RevCommit>, AutoCloseable {
	...
	private List<Ref> getMergedInto(RevCommit needle, Collection<Ref> haystacks,
		...
		for (Ref r : haystacks) {
			...
			List<Ref> refs = refsByCommit.computeIfAbsent(c, cc -> new ArrayList<>());
			refs.add(r);
		}
		monitor.update(1);
		List<RevCommit> mergedInto = getCommitsMergedInto(needle, refsByCommit.keySet(),
			returnStrategy, monitor);
		List<Ref> result = new ArrayList<>();
		for (RevCommit c : mergedInto) {
			result.addAll(refsByCommit.get(c));
		}
		return result;
	}

	private List<RevCommit> getCommitsMergedInto(RevCommit needle, Collection<RevCommit> haystacks,
			Enum returnStrategy, ProgressMonitor monitor) throws IOException {
