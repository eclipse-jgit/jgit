...
public class WorktreeAddCommand extends GitCommand<Worktree> {
	...
	public Worktree call() throws GitAPIException {
		...
		if (worktreeDir.exists() && !force) {
			throw new IllegalStateException("Directory already exists: " + path); //$NON-NLS-1$
		}
		...
		if (!orphan) {
			...
			if (commitId == null) {
				throw new IllegalStateException("Cannot resolve start point: " + resolveTarget); //$NON-NLS-1$
			}
		}
		...
		if (!force && branchToCheckout != null) {
			...
			for (Worktree wt : existingWorktrees) {
				if ((Constants.R_HEADS + branchToCheckout).equals(wt.getBranch())) {
					throw new IllegalStateException("Branch is already checked out by another worktree: " + branchToCheckout); //$NON-NLS-1$
				}
			}
		}
		...
		if (specificAdminDir.exists() && !force) {
			throw new IllegalStateException("Worktree administrative directory already exists for name: " + name); //$NON-NLS-1$
		}
