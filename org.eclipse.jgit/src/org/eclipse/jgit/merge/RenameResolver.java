 ...
 *
 * <p>
 * The {@link ResolveMerger} first processes all regular diffs, skipping any
 * diffs related to rename.
 * <p>
 * It then processes each rename pair separately, as well as rename conflicts.
 * <p>
 * This class computes the renames, introduced by merge sides and provides an
 * interface for {@link ResolveMerger} to get all renames-related information.
 *
 * <p>
 * The renames are computed related to the 'base' of the tree-way merge. If the
 * base is not present or the source file does not exist in the base commit, the
 * renames would not be detected.
 */
public class RenameResolver {
	...
	public enum RenameType {
		/**
		 * No rename is associated with this path or the type was nt computed
		 * yet.
		 */
		NO_RENAME,
		/**
		...
		RENAME_IN_THEIRS,
		/**
		 * The paths was renamed on both ours and theirs side to the same target
		 * paths, which did not result in conflicts.
		 */
		RENAME_BOTH_NO_CONFLICT,
		/**
		 * The rename was detected, by the merge under the target path is not
		 * possible.
		 */
		RENAME_CONFLICT
	}
	...
	public static class RenameEntry {
		 ...
		 * <p>
		 * This is the name of the entry in the base commit.
		 * 
		...
		/**
		 * Returns the target path of the rename, that will be updated by the
		 * merger.
		 * 
		...
		/**
		 * Returns the target path of the rename by merge side
		 * ({@link ResolveMerger#T_OURS} or {@link ResolveMerger#T_THEIRS}).
		 *
		 * <p>
		 * It is possible that {@link ResolveMerger#T_OURS} and
		 * {@link ResolveMerger#T_THEIRS}) renamed the same {@link #sourcePath}
		 * to different paths. Then results of this method are different from
		 * {@link #getTargetPath()}.
		 * 
		 * @param nth
		 *            index of the side in {@link ResolveMerger}.
		...
		/**
		 * Returns path, that (if) {@link ResolveMerger#T_OURS} and
		 * {@link ResolveMerger#T_THEIRS}) renamed {@link #getSourcePath()} to.
		 * 
		...
		/**
		 * Returns {@link RenameConflict} or {@link RenameConflict#NO_CONFLICT}
	...
	/**
	 * Map of source rename paths (present in {@link ResolveMerger#T_BASE} to
	 * target rename paths by the corresponding side
	 * trees({@link ResolveMerger#T_OURS}, {@link ResolveMerger#T_THEIRS}).
	 */
	private Map<String, RenameEntry> baseRenamePaths = new HashMap<>();

	/**
	 * For side trees ({@link ResolveMerger#T_OURS},
	 * {@link ResolveMerger#T_THEIRS}), holds the mapping of paths that were
	 * renamed from base in that side tree to their original paths in
	 * {@link ResolveMerger#T_BASE}.
	 */
	...
	/**
	 * Creates the {@link RenameResolver} holder that computes the renames for
	 * the tree-way merge.
	 *
	 * @param db
	 *            the {@link Repository} that {@link ResolveMerger} operates on.
	 * @param reader
	 *            {@link ObjectReader} that is used by {@link ResolveMerger}
	 * @param config
	 *            config of the repository.
	 * @param baseTree
	 *            a base of the merge
	 * @param headTree
	 *            {@link ResolveMerger#T_OURS} side of the merge.
	 * @param mergeTree
	 *            {@link ResolveMerger#T_THEIRS} side of the merge.
	 * @throws IOException
	 */
	public RenameResolver(@Nullable Repository db, ObjectReader reader,
			Config config, @Nullable RevTree baseTree, RevTree headTree,
	...
	private void addRenames(@Nullable RevTree baseTree, RevTree head,
		...
		List<DiffEntry> headRenames = computeRenames(renameDetector, baseTree,
				head);
		List<DiffEntry> mergeRenames = computeRenames(renameDetector, baseTree,
				merge);
		...
		for (Entry<String, RenameEntry> baseRename : baseRenamePaths
				.entrySet()) {
			RenameEntry renameEntry = baseRename.getValue();
			for (Entry<Integer, String> sideRename : renameEntry.targetPaths
					.entrySet()) {
	...
	private List<DiffEntry> computeRenames(RenameDetector renameDetector,
			RevTree baseTree, RevTree otherTree) throws IOException {
		...
		// Break down all modifies, otherwise multiple renames can not be
		// combined.
	...
	private void detectRenameConflicts(List<DiffEntry> headRenames,
		...
		Map<String, DiffEntry> headDiffByTarget = headRenames.stream()
				.filter(x -> !x.getChangeType().equals(ChangeType.DELETE))
				.collect(Collectors.toMap(x -> x.getNewPath(), x -> x));

		Set<String> mergeDeletions = mergeRenames.stream()
				.filter(x -> x.getChangeType().equals(ChangeType.DELETE))
				.map(x -> x.getOldPath()).collect(Collectors.toSet());
		Map<String, DiffEntry> mergeDiffByTarget = mergeRenames.stream()
				.filter(x -> !x.getChangeType().equals(ChangeType.DELETE))
				.collect(Collectors.toMap(x -> x.getNewPath(), x -> x));
		...
		for (Entry<String, RenameEntry> baseRename : baseRenamePaths
			...
			if (renames.containsKey(T_OURS) && !renames.containsKey(T_THEIRS)) {
				offSourceRenames.addAll(detectRenameConflicts(
						baseRename.getKey(), renames.get(T_OURS),
						mergeDiffByTarget, mergeDeletions));
			} else if (renames.containsKey(T_THEIRS)
					&& !renames.containsKey(T_OURS)) {
				offSourceRenames.addAll(detectRenameConflicts(
						baseRename.getKey(), renames.get(T_THEIRS),
						headDiffByTarget, headDeletions));
			} else if (!renames.get(T_OURS).equals(renames.get(T_THEIRS))) {
				// Attempt to resolve the conflict: if source path is still
				// present on the side, that renamed this path, do not recognize
				// that this side as a rename. On this side, treat it as a
				// regular add. See test cases for the rationale.
				if (mergeDiffByTarget.containsKey(baseRename.getKey())
					...
					// If the target renames side was modified on the opposite
					// side, this is also not resolvable.
					offSourceRenames.add(baseRename.getKey());
				} else {
					// we are attempting to rename same source to different
					// things
					// Only report as rename conflict if there is no correlation
					// on the sides.
	...
	private Set<String> detectRenameConflicts(String sourcePath,
			String targetPath,
			Map<String, DiffEntry> otherSideDiffsByTargetPath,
		...
		DiffEntry otherSideTargetDiffEntry = otherSideDiffsByTargetPath
				.get(targetPath);
		// The other entry has the target rename path and it's source does not
		// match the source of the rename.
		if (!otherSideTargetDiffEntry.getOldPath().equals(sourcePath)) {
			if (!otherSideTargetDiffEntry.getChangeType()
				...
				// Add and modify counts as conflict. If the entry is present
				// but was not changed comparing to base, it can be safely
				// dropped.
				recordSourceConflict(sourcePath,
	 ...
	 * 
	 * @param base
	 *            base tree of three-way merge.
	 * @return true if the base tree is positioned at the source path of the
	 *         rename.
	 */
	 ...
	 * 
	 * @param nth
	 *            index of the tree in three-way merge
	 *            ({@link ResolveMerger#T_OURS} or
	 *            {@link ResolveMerger#T_THEIRS}).
	 * @param side
	 *            side tree of the three-way merge.
	 ...
	 * @param path
	 *            path string to check.
	 * @return path true if the path the target of the rename in any of the side
	 *         trees.
	 */
	...
	/**
	 * Returns if any of the trees are positioned at the path, that is involved
	 * into rename (eiter source or target).
	 *
	 * @param base
	 *            {@link ResolveMerger#T_BASE} {@link AbstractTreeIterator} on
	 *            the three-way merge.
	 * @param ours
	 *            {@link ResolveMerger#T_OURS} {@link AbstractTreeIterator} on
	 *            the three-way merge.
	 * @param theirs
	 *            {@link ResolveMerger#T_THEIRS} {@link AbstractTreeIterator} on
	 *            the three-way merge.
	 * @return if the entry that the trees are positioned is involved into
	 *         rename (eiter source or target).
	 */
	public boolean isRenameEntry(AbstractTreeIterator base,
			AbstractTreeIterator ours, AbstractTreeIterator theirs) {
		return isSourceOfRename(base) || isTargetOfRename(T_OURS, ours)
				|| isTargetOfRename(T_THEIRS, theirs);
	}
	 ...
	 * 
	 * @param sourcePath
	 *            path to return rename type for.
	 * @return {@link RenameType}.
	 */
	 ...
	 * 
	 * @param sourcePath
	 *            path to return rename entry for.
	...
	/**
	 * Returns {@link RenameEntry} for a detected rename conflict by its
	 * {@code sourcePath}.
	 * 
	 * @param sourcePath
	 *            path to return rename entry for.
	 ...
	 * <p>
	 * See {@link RenameConflict} for the recognized conflicts.
	 * 
