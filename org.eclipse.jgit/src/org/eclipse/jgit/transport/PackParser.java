...
public abstract class PackParser {
	...
	private void indexOneObject() throws IOException {
		...
		case Constants.OBJ_TAG:
			...
			// Difference between the current stream position and the previous
			// stream position is the size of the header.
			whole(streamPosition, typeCode, sz, streamPosition() - streamPosition);
			break;
		...
		case Constants.OBJ_OFS_DELTA: {
			...
			long headerSize = streamPosition() - streamPosition;
			...
			// Difference between the current number of compressed bytes read by
			// the inflater and the previous number of compressed bytes read by
			// the inflater is the size of the body. Size of the delta object
			// equals header size plus body size.
