...
public class ConfigAwareCredentialsProvider extends CredentialsProvider {
	...
	/**
	 * Creates a provider that merges the given config with the default provider.
	 * Any credentials configured in the given config will take precedence over
	 * the default provider.
	 * @param config the effective config to resolve credential settings from.
	 * @return a provider that merges the given config with the default provider
	 */
	public static CredentialsProvider mergeWithDefault(Config config) {
		ConfigAwareCredentialsProvider configAwareProvider = new ConfigAwareCredentialsProvider(config);
		CredentialsProvider defaultProvider = CredentialsProvider.getDefault();
		if (defaultProvider == null) {
			return configAwareProvider;
		}
		return new ChainingCredentialsProvider(configAwareProvider, defaultProvider);
	}

	/**
	 * Creates a provider that merges the user config with the default provider.
	 * Any credentials configured in config will take precedence over
	 * the default provider.
	 * Only use this when no repository is available to provide a config, otherwise use {@link #mergeWithDefault(Config)}.
	 * @return a provider that merges the given config with the default provider
	 * @throws IOException if something went wrong when reading config files
	 * @throws ConfigInvalidException if the config is invalid
	 */
	public static CredentialsProvider mergeWithDefault() throws ConfigInvalidException, IOException {
		return mergeWithDefault(SystemReader.getInstance().getUserConfig());
	}
