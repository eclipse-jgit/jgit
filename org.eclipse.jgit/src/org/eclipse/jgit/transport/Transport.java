...
public abstract class Transport implements AutoCloseable {
	...
	public FetchResult fetch(final ProgressMonitor monitor,
			...
			throws NotSupportedException, TransportException {
		return fetch(monitor, toFetch, null, refSpec -> true);
	}
	 ...
	 *            empty collection to use the specifications from the
	 *            RemoteConfig. Source for each RefSpec can't be null.
	 * @param allowRefSpecFilter
	 *            refSpec filter to filter out refSpecs in {@code toFetch}. 
	 *            Only refSpecs meets certain requirements will be fetched. May 
	 *            be null to fetch every refSpec in {@code toFetch}
	 * @return information describing the tracking refs updated.
	 * @throws org.eclipse.jgit.errors.NotSupportedException
	 *             this transport implementation does not support fetching
	 *             objects.
	 ...
	 */
	public FetchResult fetch(final ProgressMonitor monitor,
			Collection<RefSpec> toFetch, Predicate<RefSpec> allowRefSpecFilter)
			throws NotSupportedException, TransportException {
		return fetch(monitor, toFetch, null, allowRefSpecFilter);
	}
	...
	public FetchResult fetch(final ProgressMonitor monitor,
			...
			throws NotSupportedException, TransportException {
		return fetch(monitor, toFetch, branch, refSpec -> true);
	}
