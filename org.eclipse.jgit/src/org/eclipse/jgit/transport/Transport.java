...
public abstract class Transport implements AutoCloseable {
	...
	public FetchResult fetch(final ProgressMonitor monitor,
			...
			throws NotSupportedException, TransportException {
		return fetch(monitor, toFetch, null, null);
	}
	 ...
	 *            empty collection to use the specifications from the
	 *            RemoteConfig. Source for each RefSpec can't be null.
	 * @param negativeRefSpecs
	 *            specification of refs not to fetch. May be null or the empty
	 *            collection to not skip any fetch. the {@link RefSpec}s must
	 *            have source only which means that not to fetch from this
	 *            source branch or destination only which means that not to
	 *            fetch to this destination branch. Wildcard patterns are also
	 *            supported in both sources and destinations.
	 * @return information describing the tracking refs updated.
	 * @throws org.eclipse.jgit.errors.NotSupportedException
	 *             this transport implementation does not support fetching
	 *             objects.
	 ...
	 */
	public FetchResult fetch(final ProgressMonitor monitor,
			Collection<RefSpec> toFetch, Collection<RefSpec> negativeRefSpecs)
			throws NotSupportedException, TransportException {
		return fetch(monitor, toFetch, null, negativeRefSpecs);
	}
	...
	public FetchResult fetch(final ProgressMonitor monitor,
			Collection<RefSpec> toFetch, String branch,
			Collection<RefSpec> negativeRefSpecs)
			throws NotSupportedException, TransportException {
		...
		new FetchProcess(this, toFetch, negativeRefSpecs).execute(monitor, result, branch);
