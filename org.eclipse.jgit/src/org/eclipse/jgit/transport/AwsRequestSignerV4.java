...
public final class AwsRequestSignerV4 {
	...
	static {
		AMZ_DATE_FORMAT = new SimpleDateFormat("yyyyMMdd'T'HHmmss'Z'"); //$NON-NLS-1$
		AMZ_DATE_FORMAT.setTimeZone(new SimpleTimeZone(0, "UTC")); //$NON-NLS-1$
		SCOPE_DATE_FORMAT = new SimpleDateFormat("yyyyMMdd"); //$NON-NLS-1$
		SCOPE_DATE_FORMAT.setTimeZone(new SimpleTimeZone(0, "UTC")); //$NON-NLS-1$
	}
	 ...
	 * header.
	 *
	 * @param httpURLConnection The request to sign.
	 * @param queryParameters   The query parameters being sent in the request.
	 * @param contentLength     The content length of the data being sent in the
	 *                          request
	 * @param bodyHash          Hex-encoded SHA-256 hash of the data being sent in
	 *                          the request
	 * @param serviceName       The signing name of the AWS service (e.g. "s3").
	 * @param regionName        The name of the AWS region that will handle the
	 *                          request (e.g. "us-east-1").
	 * @param awsAccessKey      The user's AWS Access Key.
	 * @param awsSecretKey      The user's AWS Secret Key.
	 */
	public static void sign(HttpURLConnection httpURLConnection, Map<String, String> queryParameters,
			long contentLength, String bodyHash, String serviceName, String regionName, String awsAccessKey,
			char[] awsSecretKey) {
		// get request headers
		Map<String, String> headers = new HashMap<>();
		httpURLConnection.getRequestProperties()
				.forEach((headerName, headerValues) -> headers.put(headerName, String.join(",", headerValues))); //$NON-NLS-1$

		// add required content headers
		if (contentLength > 0) {
			headers.put(HttpSupport.HDR_CONTENT_LENGTH, String.valueOf(contentLength));
		} else {
		...
		String hostHeader = (port > -1) ? endpointUrl.getHost().concat(":" + port) //$NON-NLS-1$
				: endpointUrl.getHost();
		...
		String authorizationHeader = SCHEME + '-' + ALGORITHM + ' ' + credentialsAuthorizationHeader + ", " //$NON-NLS-1$
		...
		httpURLConnection.setRequestProperty(HttpSupport.HDR_AUTHORIZATION, authorizationHeader);
	}
	 ...
	 *
	 * @param headers Map containing all request headers.
	 *
	...
	private static String getCanonicalizeHeaderNames(Map<String, String> headers) {
		return headers.keySet().stream().map(String::toLowerCase).sorted().collect(Collectors.joining(";")); //$NON-NLS-1$
	}
	 ...
	 *
	 * @param url The AWS service endpoint URL, including the path to any resource.
	 *
	 ...
	 *
	 * @param queryParameters The query parameters in the request.
	 *
	...
	public static String getCanonicalizedQueryString(Map<String, String> queryParameters) {
		...
		return queryParameters.keySet().stream().sorted().map(key -> HttpSupport.urlEncode(key, false) + "=" //$NON-NLS-1$
				+ HttpSupport.urlEncode(queryParameters.get(key), false)).collect(Collectors.joining("&")); //$NON-NLS-1$
	}
