...
abstract class BasePackConnection extends BaseConnection {
	...
	private void lsRefsImpl(Collection<RefSpec> refSpecs,
		...
		// Merge in advertised symrefs if the ref was also advertised
		for (Map.Entry<String, String> symRef : symRefs.entrySet()) {
			Ref existing = avail.get(symRef.getValue());
			if (existing != null) {
				avail.put(symRef.getKey(),
						new SymbolicRef(symRef.getKey(), existing));
			}
		}
		available(avail);
	}
