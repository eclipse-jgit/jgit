...
public final class CommandInfo {
	...
	/** Parsed repo path from the command line, or null if not present. */
	public final String repo;

	/**
	 * Git command (e.g. git-upload-pack or git-receive-pack), or null if not
	 * recognized.
	 */
	 ...
	 *
	 * @param client
	 *            client socket providing the pkt-line
	 * @throws IOException
	 *             if reading the pkt-line fails
	 */
	public CommandInfo(Socket client) throws IOException {
		RecordingInputStream stream = new RecordingInputStream(
		...
		extraParameters = nulnul != -1
				? Arrays.asList(line.substring(nulnul + 2).split("\0")) //$NON-NLS-1$
				: List.of();

		String resolvedCommand = null;
		String resolvedArgument = null;
		for (String candidate : new String[] { RemoteConfig.DEFAULT_UPLOAD_PACK,
	 ...
	 * <p>
	 * Example input: "git-receive-pack /example/repo.git\0" returns
	 * "example/repo.git".
	 *
	 * @param line
	 *            packet line payload
	 * @param command
	 *            command prefix to search
	 ...
	 */
	private static String sanitizeRepoName(String in) {
		int start = 0;
		...
		if (result.endsWith(".git")) { //$NON-NLS-1$
