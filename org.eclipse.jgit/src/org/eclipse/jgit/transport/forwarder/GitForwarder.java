...
public class GitForwarder implements AutoCloseable {
	...
	/**
	 * Creates a forwarder that listens on the given host and the default git
	 * daemon port ({@link Daemon#DEFAULT_PORT}).
	 *
	 * @param listenHost
	 *            host to bind
	 * @param listener
	...
	public GitForwarder(@NonNull String listenHost, @NonNull RoutingListener listener) {
		this(listenHost, Daemon.DEFAULT_PORT, listener, Executors.newCachedThreadPool());
	}
	...
	private void handleConnection(Socket clientSocket) {
		...
		try {
			...
			LOG.info(JGitText.get().forwarderIncomingConnection,
					...
					commandInfo.host,
					request.listenHost(),
					Integer.valueOf(request.listenPort()));
		} catch (IOException e) {
	...
	private void proxyBidirectional(Socket in, Socket out, byte[] prefix, Consumer<Exception> onException) {
		...
		AtomicReference<Future<?>> forwardFuture = new AtomicReference<>();
		AtomicReference<Future<?>> responderFuture = new AtomicReference<>();

		forwardFuture.setPlain(workerPool.submit(() ->
				forward(in, out, prefix, done, responderFuture::getPlain, onException)));
		responderFuture.setPlain(workerPool.submit(() ->
				forward(out, in, null, done, forwardFuture::getPlain, onException)));

		try {
			forwardFuture.getPlain().get();
			responderFuture.getPlain().get();
