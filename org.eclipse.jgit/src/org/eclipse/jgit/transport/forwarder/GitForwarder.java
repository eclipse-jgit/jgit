...
public class GitForwarder implements AutoCloseable {
	 ...
	 * @throws IOException on failure to bind to the socket or on writing/reading to/from the socket
	 */
	...
	private void startInternal() throws IOException {
		try {
			...
			LOG.error(MessageFormat.format(JGitText.get().forwarderFailedToStart, listenOn), e);
			throw e;
		}
	}
	...
	private void acceptLoop() {
		while (!serverSocket.isClosed()) {
			try {
					...
					try {
						...
						LOG.error(
							MessageFormat.format(
								JGitText.get().forwarderFailedToParseCommandInfo, clientSocket.getInetAddress()), e);
          } finally {
				...
				// This could happen when client closes abruptly just doing a heartbeat check.
			}
		}
	}
	...
	private void handleConnection(RouteRequest request) {
		...
		try {
			...
			LOG.error(
				MessageFormat.format(
					JGitText.get().forwarderFailedConnection, upstreamSocket.getInetAddress()), e);
	...
	private void proxyBidirectional(Socket in, Socket out, byte[] prefix, Consumer<Exception> onException) {
		...
		try {
			...
			LOG.debug(
				MessageFormat.format(
					JGitText.get().forwarderFailedToPipe, in.getInetAddress(), out.getInetAddress()), e);
	...
	private void forward(
		...
		try {
			...
			// When latch is counted down, the exception s recorded by other the thread. Avoid duplicate.
			if (latch.getCount() > 0) {
				LOG.error(
						MessageFormat.format(
	...
	public void close(long timeout, TimeUnit unit) {
		try {
			...
			LOG.debug(JGitText.get().forwarderFailedToCloseSocket, e);
		}
