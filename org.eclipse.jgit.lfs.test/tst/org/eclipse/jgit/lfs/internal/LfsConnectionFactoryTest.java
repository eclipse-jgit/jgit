...
public class LfsConnectionFactoryTest extends RepositoryTestCase {
	...
	public void setUp() throws Exception {
		...
		git.remoteAdd().setName(DEFAULT_REMOTE_NAME)
				.setUri(new URIish(ORIGIN_URL)).call();
	}
	...
	/*
	 * Write simple lfs config with single entry.
	 * Do not use FileBasedConfig to avoid introducing new
	 * dependency (for now).
	 */
	...
	@Test
	public void checkGetLfsConnection_remote_WorkingDir() throws Exception {
		writeLfsConfig(LFS_SERVER_URL1, "lfs", DEFAULT_REMOTE_NAME, "url");
		checkLfsUrl(LFS_SERVER_URL1 + Protocol.INFO_LFS_ENDPOINT);
	}
