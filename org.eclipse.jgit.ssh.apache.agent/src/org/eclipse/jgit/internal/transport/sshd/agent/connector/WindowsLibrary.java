...
/**
 * The {@link WindowsLibrary} encapsulates all Windows-specific stuff.
 */
public final class WindowsLibrary {
	...
	private static class PipeImpl implements Pipe {
		...
		public void close() throws IOException {
			WindowsLibrary.close(libs, file, memory, false);
		}
		...
		public void send(byte[] message) throws IOException {
			...
			if (result == null || result.longValue() == 0) {
				throw new IOException(
						systemError(libs, Texts.get().msgSendFailed2));
			}
		}
	...
	Pipe createPipe(String name, int maxSize) throws IOException {
		...
		try {
			...
			if (file == null || file == WinBase.INVALID_HANDLE_VALUE) {
				throw new IOException(
						systemError(libs, Texts.get().msgNoMappedFile));
			}
			sharedMemory = libs.kernel.MapViewOfFile(file,
					WinNT.SECTION_MAP_WRITE, 0, 0, 0);
			if (sharedMemory == null) {
				throw new IOException(
						systemError(libs, Texts.get().msgNoSharedMemory));
			}
	...
	private static void close(LibraryHolder libs, HANDLE file, Pointer memory,
		...
		if (file != null) {
			if (!libs.kernel.CloseHandle(file)) {
				String msg = systemError(libs, Texts.get().errCloseMappedFile);
				if (silent) {
					LOG.error(msg);
				} else {
					throw new IOException(msg);
	...
	private static String systemError(LibraryHolder libs, String pattern) {
