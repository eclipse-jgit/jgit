...
format. Currently, there are 2 versions known, and they only differ in the size
of the hash. `format_version=1` indicates the SHA1 hash, so all hash values are
20 bytes wide. `format_version=2` indicates SHA256, with 32 byte hash values.
