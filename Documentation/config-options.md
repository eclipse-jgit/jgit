...
| `core.trustFolderStat` | `true` | &#x20DE; | Whether to trust the pack folder's and packed-refs file's modification time. Note that, for packed-refs file, `trustFolderStat` comes into effect only when `trustPackedRefsModificationTime` is `unset`. When looking for new pack files, if `false` JGit will always scan the `.git/objects/pack` folder and if set to `true` it uses the `lastmodified` attribute of the pack folder and assumes that no new pack files can be in this folder if its modification time has not changed. When getting list of packed refs, if `false` (and `core.trustPackedRefsModificationTime` is `unset`) JGit will always read the packed-refs file and if set to `true` (and `core.trustPackedRefsModificationTime` is `unset`) it uses the `lastmodified` attribute of the packed-refs file and will only read it if its modification time has changed. Setting this option to `false` can help to workaround caching issues on NFS, but reduces performance. |
| `core.trustPackedRefsModificationTime` | `unset` | &#x20DE; | Whether to trust the modification time of packed-refs file. If `never` JGit will ignore the modification time of the packed-refs file and always read it. If `always` JGit will trust the modification time of packed-refs file and will only read it if its modification time has changed. `after_open` behaves the same as `always`, except that the packed-refs file is opened and closed before its modification time is considered. An open/close of the packed-refs file is known to refresh its modification time at least on some NFS clients. If `unset`, JGit will always read the packed-refs file only when `trustFolderStat` is `false`. |
