...
| `core.trustLooseRefStat` | `always` | &#x20DE; | Whether to trust the file attributes (Java equivalent of stat command on *nix) of the loose ref. If `always` JGit will trust the file attributes of the loose ref. `after_open` behaves the same as `always`, except that all parent directories of the loose ref until the repository root are opened and closed before its file attributes are considered. An open/close of these parent directories is known to refresh its file attributes, at least on some NFS clients. |
