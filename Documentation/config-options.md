...
| `core.refCache`  | `false` | &#x20DE; | If set to `true` and `extensions.refStorage` is unset or set to `refdir` all refs are cached in a write-through cache to speedup ref lookup. According to tests the cache allocates 200-600 bytes memory per ref. __Important note:__ when this cache is enabled jgit does not detect if refs stored on disk are modified by another process. |
