...
| `core.trustFolderStat` | `true` | &#x20DE; | Whether to trust the packed-refs file modification time. If `false` JGit will always re-read the entire packed-refs file. This can help to workaround caching issues on NFS, but reduces performance. If set to `true` it uses the `lastmodified` attribute of the file and assumes that the file has not changed if its modification time has not changed. |
