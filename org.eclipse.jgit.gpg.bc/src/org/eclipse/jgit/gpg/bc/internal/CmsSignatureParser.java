...
class CmsSignatureParser implements SignatureParser {

	private static final int BEGIN_DELIMITER_LEN = Constants.CMS_SIGNATURE_PREFIX.length() + 1;

	private static final CmsSignatureParser SINGLETON = new CmsSignatureParser();

	static void register() {
		SignatureParserProvider.registerSignatureParser(Constants.CMS_SIGNATURE_PREFIX, SINGLETON);
	}
	...
	public GpgSignatureVerifier.SignatureVerification verify(GpgConfig config, byte[] data, byte[] signatureData) throws IOException {
		byte[] signedContent = new byte[signatureData.length - BEGIN_DELIMITER_LEN];
		...
		try {
			...
			return new VerificationResult(signingTime,
				...
				GpgSignatureVerifier.TrustLevel.UNKNOWN,
				null);
		} catch (Exception e) {
			throw new IOException(BCText.get().cmdSignatureVerificationFailed, e);
		}
	}
