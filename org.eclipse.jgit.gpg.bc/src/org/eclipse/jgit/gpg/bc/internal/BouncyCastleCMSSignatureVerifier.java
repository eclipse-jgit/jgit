...
public class BouncyCastleCMSSignatureVerifier extends BouncyCastleAbstractGpgSignatureVerifier {
  ...
  public BouncyCastleCMSSignatureVerifier() {
  ...
  public SignatureVerification verify(byte[] data, byte[] signatureData) throws IOException {
    ...
    System.arraycopy(signatureData, BEGIN_DELIMITER_LEN, signedContent, 0, signedContent.length);
    try {
      CMSSignedData cmsSignedData =
          new CMSSignedData(new CMSProcessableByteArray(data), Base64.decode(signedContent));
      SignerInformation signerInfo = cmsSignedData.getSignerInfos().getSigners().iterator().next();
      X509CertificateHolder certificateHolder =
          (X509CertificateHolder)
              cmsSignedData.getCertificates().getMatches(signerInfo.getSID()).iterator().next();
