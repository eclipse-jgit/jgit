...
public class BouncyCastleGpgSignatureVerifier implements GpgSignatureVerifier {
	...
	public SignatureVerification verifySignature(@NonNull RevObject object)
		...
		if (object instanceof RevCommit) {
			...
			signatureData = commit.getRawGpgSignature();
			...
			data = new byte[raw.length - (end - start)];
			...
			signatureData = tag.getRawGpgSignature();
			if (signatureData == null) {
				return null;
			}
	...
	private SignatureVerification verify(byte[] data, byte[] signatureData)
			throws IOException {
