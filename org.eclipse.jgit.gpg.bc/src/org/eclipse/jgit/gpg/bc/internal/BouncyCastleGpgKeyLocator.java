...
public class BouncyCastleGpgKeyLocator {
	...
	static byte[] getKeyGrip(PGPPublicKey publicKey)
		...
		case PublicKeyAlgorithmTags.EDDSA:
			...
			if (OID_OPENPGP_ED25519.equals(curveOID.getId())
				...
				return hash_ed25519(grip, ec.getEncodedPoint());
				...
				return hash_curve25519(grip, ec.getEncodedPoint());
			}
			...
			if (publicKey.getAlgorithm() == PublicKeyAlgorithmTags.EDDSA) {
				...
				hash(grip, Arrays.copyOfRange(data, 1, data.length), 'q',
						false);
			} else {
	...
	private static byte[] hash_ed25519(SHA1 grip, BigInteger q) {
		// For the values, see RFC 7748: https://tools.ietf.org/html/rfc7748
		// p = 2^255 - 19
		hash(grip, Hex.decode(
		...
		hash(grip, Hex.decodeStrict(
				...
				'n', false);
		// An EdDSA public key is always in compressed format, and should start
		// with 0x40, indicating compression. libgcrypt always skips the
		// compression flag when computing the keygrip.
		byte[] data = q.toByteArray();
	...
	@SuppressWarnings("nls")
	private static byte[] hash_curve25519(SHA1 grip, BigInteger q) {
		hash(grip, Hex.decode(
				"7FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFED"),
				'p', false);
