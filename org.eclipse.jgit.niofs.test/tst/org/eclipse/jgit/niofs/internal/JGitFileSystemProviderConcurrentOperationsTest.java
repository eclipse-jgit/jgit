...
public class JGitFileSystemProviderConcurrentOperationsTest extends BaseTest {
	...
	public void testConcurrentGitCreation() {
		...
		for (int i = 0; i < threadCount; i++) {
			final int name = i;
			Runnable r = () -> {
				provider.createNewGitRepo(EMPTY_ENV, "git://parent/concurrent-test" + name);
				finished.countDown();
	...
	public void testConcurrentGitDeletion() throws IOException {
		...
		for (int i = 0; i < threadCount; i++) {
			...
			Runnable r = () -> {
				try {
					provider.deleteFS(fs.getRealJGitFileSystem());
				} catch (IOException e) {
