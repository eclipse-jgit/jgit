...
public class PseudorandomTest extends CLIRepositoryTestCase {
	...
	private File expectedDbDir;

	private Repository expectedDb;

	@Override
	@Before
	public void setUp() throws Exception {
		super.setUp();
		File expectedDbGitDir = createUniqueTestGitDir(false);
		expectedDbDir = expectedDbGitDir.getParentFile();
		expectedDb = createRepository(expectedDbGitDir);
		initRepository();
	}

	private void initRepository() throws IOException, GitAPIException {
		Repository[] dbs = new Repository[] { db, expectedDb };
