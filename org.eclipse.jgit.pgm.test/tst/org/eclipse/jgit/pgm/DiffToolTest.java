...
public class DiffToolTest extends ToolTestCase {
	...
	void testUserToolWithCommandNotFoundError() throws Exception {
		assertThrows(Die.class, () -> {
			...
			createMergeConflict();
			runAndCaptureUsingInitRaw(DIFF_TOOL, "--no-prompt", "--tool",
					toolName);
	...
	@Test
	void testToolWithPrompt() throws Exception {
		String[] inputLines = { "y", // accept launching diff tool
				"y", // accept launching diff tool
		};

		String[] conflictingFilenames = createUnstagedChanges();
		String[] expectedOutput = getExpectedCompareOutput(
	...
	@Test
	void testToolAbortLaunch() throws Exception {
		String[] inputLines = { "y", // accept launching diff tool
	...
	@Test
	void testNotDefinedTool() throws Exception {
		assertThrows(Die.class, () -> {
			createUnstagedChanges();
	...
	void testTool() throws Exception {
		...
		String[] expectedOutput = getExpectedToolOutputNoPrompt(
				conflictFilenames);
		...
		for (String option : options) {
			assertArrayOfLinesEquals("Incorrect output for option: " + option,
					expectedOutput,
					runAndCaptureUsingInitRaw(DIFF_TOOL, option, TOOL_NAME));
		}
	}

	@Test
	void testToolTrustExitCode() throws Exception {
		String[] conflictingFilenames = createUnstagedChanges();
		String[] expectedOutput = getExpectedToolOutputNoPrompt(
				conflictingFilenames);
	...
	@Test
	void testToolNoGuiNoPromptNoTrustExitcode() throws Exception {
		String[] conflictingFilenames = createUnstagedChanges();
		String[] expectedOutput = getExpectedToolOutputNoPrompt(
				conflictingFilenames);
		...
		for (String option : options) {
			assertArrayOfLinesEquals("Incorrect output for option: " + option,
					expectedOutput,
					runAndCaptureUsingInitRaw(DIFF_TOOL, "--no-gui",
							"--no-prompt", "--no-trust-exit-code", option,
							TOOL_NAME));
		}
	}

	@Test
	void testToolCached() throws Exception {
		String[] conflictingFilenames = createStagedChanges();
		Pattern[] expectedOutput = getExpectedCachedToolOutputNoPrompt(
				conflictingFilenames);
		...
		for (String option : options) {
			assertArrayOfMatchingLines("Incorrect output for option: " + option,
					expectedOutput, runAndCaptureUsingInitRaw(DIFF_TOOL, option,
							"--tool", TOOL_NAME));
		}
	}
	...
	private String[] getExpectedToolOutputNoPrompt(
			String[] conflictingFilenames) {
