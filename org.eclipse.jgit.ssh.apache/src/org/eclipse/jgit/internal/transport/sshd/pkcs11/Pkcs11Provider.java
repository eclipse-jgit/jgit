	...
	public static Pkcs11Provider getProvider(@NonNull Path library,
		...
		try {
			...
			return PROVIDERS.computeIfAbsent(key, sharedLib -> {
				...
				String name = "JGit-" + slotIndex + '-' + libPath; //$NON-NLS-1$
				try {
					Path config = Files.createTempFile("jgit", "pkcs11"); //$NON-NLS-1$ //$NON-NLS-2$
					Files.write(config, List.of("name = " + name, //$NON-NLS-1$
							"library = " + libPath, //$NON-NLS-1$
							"slotListIndex = " + slotIndex)); //$NON-NLS-1$
					config.toFile().deleteOnExit();
					if (LOG.isDebugEnabled()) {
						// No translations for debug logging.
						LOG.debug("{}: config file {}", name, config); //$NON-NLS-1$
					}
		...
		while (aliases.hasMoreElements()) {
			...
			if (LOG.isDebugEnabled()) {
				if (certificate instanceof X509Certificate) {
					...
					msg += '.';
					LOG.debug("{}: Loaded X.509 certificate {}, key type {}.{}", //$NON-NLS-1$
