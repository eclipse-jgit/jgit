...
public class Pkcs11Provider {
	...
	private static final Map<Path, Pkcs11Provider> PROVIDERS = new ConcurrentHashMap<>();
	...
	public static Pkcs11Provider getProvider(@NonNull Path library,
		...
		try {
			return PROVIDERS.computeIfAbsent(library, sharedLib -> {
				Provider pkcs11 = Security.getProvider("SunPKCS11"); //$NON-NLS-1$
				if (pkcs11 == null) {
					throw new UnsupportedOperationException();
				}
				// Write a temp file with the configuration
				int slot = slotListIndex < 0 ? 0 : slotListIndex;
				String name = "JGit-" + slot + '-' + library; //$NON-NLS-1$
				try {
					Path config = Files.createTempFile("jgit", "pkcs11"); //$NON-NLS-1$ //$NON-NLS-2$
					Files.write(config, List.of("name = " + name, //$NON-NLS-1$
							"library = " + library.toAbsolutePath(), //$NON-NLS-1$
							"slotListIndex = " + slot)); //$NON-NLS-1$
					config.toFile().deleteOnExit();
					pkcs11 = pkcs11.configure(config.toString());
				...
				// the query.
				String path = "pkcs11:?module-path=" + library.toString(); //$NON-NLS-1$
				if (slotListIndex > 0) {
	...
	// sign something. Therefore we initialize the prompter in both cases, even
	// if it may be unused in one or the other operation.
	//
