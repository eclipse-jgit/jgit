...
//		boolean useRefTable = false;
		...
		@Param({ "~/tmp/git/jgit.git", "~/tmp/git/gerrit.git",
				"~/tmp/git/go.git", "~/tmp/git/large-repo-sap01.git",
	...
	@Warmup(iterations = 2, time = 1, timeUnit = TimeUnit.MILLISECONDS)
	@Measurement(iterations = 2, time = 10, timeUnit = TimeUnit.SECONDS)
	public void testGetExactRefCached(Blackhole blackhole, BenchmarkState state)
			throws IOException {
		String refName = state.getRandomRefName();
		blackhole.consume(state.refCache.exactRef(refName));
	}
	...
	public void testGetRefsByPrefix(Blackhole blackhole, BenchmarkState state)
		...
		blackhole.consume(state.repo.getRefDatabase()
				.getRefsByPrefix(randomChangePrefix));
	}
	...
	@Measurement(iterations = 2, time = 10, timeUnit = TimeUnit.SECONDS)
	public void testGetRefsByPrefixCached(Blackhole blackhole,
			BenchmarkState state) throws IOException {
		String randomChangePrefix = state.getRandomRefsPrefix();
		blackhole.consume(state.refCache.getRefsByPrefix(randomChangePrefix));
	}
